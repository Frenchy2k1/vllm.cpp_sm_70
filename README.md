...
| Unified streaming parser engine | Core, assembly and serving-SSE dispatch landed, gated | The vLLM 0.26 declarative `parser/engine/` (shared state machine plus qwen3/seed_oss/kimi_k2 configs) and its assembly layer, gated field-for-field vs vLLM 0.26. Wired into the OpenAI chat SSE path: an engine-backed `--tool-call-parser` name drives the live chunks, gated chunk-for-chunk. Off by default; other dialects keep the per-family path. Details: .agents/specs/parser-assembly-c8.md |
