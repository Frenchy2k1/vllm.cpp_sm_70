...
| Metal | Apple Silicon | Two models run end to end and pass correctness (OPT-125m, Qwen3-0.6B); 18 of 75 ops native, rest fall back to CPU on unified memory. Batched command buffers (1.50x at b=1) leave it **compute bound at 98%+**, so the GEMM is now the lever. With the optional MLX GEMM provider (`-DVLLM_CPP_MLX=ON`), b=1 is 9.4 tok/s vs MLX-LM's 27.9. Indicative ([BENCHMARKS](docs/BENCHMARKS.md)) |
