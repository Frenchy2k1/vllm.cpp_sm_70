...
| Metal | Apple Silicon | Two models run end to end and pass correctness (OPT-125m, Qwen3-0.6B); 18 of 75 ops native, rest fall back to CPU on unified memory. Batched command buffers, a decode GEMV, a simdgroup-matrix GEMM and typed loads put b=1 at 15.3 tok/s vs MLX-LM's 27.9. Our GEMM runs ~1.5 TFLOP/s against MLX's ~3.3; four causes for the gap are excluded by measurement. Indicative ([BENCHMARKS](docs/BENCHMARKS.md)) |
