...
Speculative decoding (MTP, k=1) is available on the Qwen3.5/3.6 checkpoints via `--speculative-config`, at concurrency 1 (token-exact and faster than vLLM) and above (mixed-batch concurrency, on par or above vLLM). It is end-to-end token-exact vs vLLM 0.25.0 on BOTH gate models: the 27B GDN hybrid (`Qwen3_5MTP`) and the 35B MoE (`Qwen3_5MoeMTP`), each three-way identical (our spec-ON == our spec-OFF == vLLM `--speculative-config mtp` greedy) with the draft head alive (acceptance matched to vLLM). DFlash speculative decoding remains oracle-blocked on the pinned 0.25.0 (vllm#40898); a go/no-go spike (2026-07-26) validated a staged next-pin target stack (vLLM main `55596792` + transformers 5.14.1 + torch 2.13) on GB10 where the DFlash mixed-attention draft now constructs, runs, and shows non-zero acceptance (2.2-8.8x), and the Gemma-4 / OLMo-3 oracles unblock; a follow-up RCA (2026-07-26) then proved the target stack reproduces every measured SACRED golden bit-for-bit under apples-to-apples capture (the earlier apparent 27B NVFP4 drift was a documented whitespace near-tie flipping on incidental engine config, not real drift), so no golden re-capture and no kernel re-sync are needed; the pin is not yet advanced (see [`.agents/specs/pin-advance.md`](.agents/specs/pin-advance.md)). Not yet supported: multimodal / vision, LoRA, multi-GPU, and the full tool-calling template surface. See [Serving and API notes](#serving-and-api-notes).
