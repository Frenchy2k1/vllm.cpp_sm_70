...
| Metal | Apple Silicon | Two models run end to end and pass correctness (OPT-125m, Qwen3-0.6B); 18 of 75 ops native, rest fall back to CPU on unified memory. Batched command buffers (1.50x) then a decode GEMV (1.94x) put b=1 at 10.5 tok/s vs MLX-LM's 27.9. Prefill still uses the tile GEMM and is 34% of GPU time. Optional MLX GEMM provider via `-DVLLM_CPP_MLX=ON`. Indicative ([BENCHMARKS](docs/BENCHMARKS.md)) |
