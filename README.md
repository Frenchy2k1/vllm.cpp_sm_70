...
| Unified streaming parser engine | Core plus assembly landed, gated, not yet the live path | The vLLM 0.26 declarative `parser/engine/` (shared streaming state machine plus qwen3/seed_oss/kimi_k2 configs) and its assembly layer that turns the event stream into streaming tool-call deltas and one-shot extracted tool calls, gated field-for-field vs vLLM 0.26. Additive and opt-in; the per-family parsers above stay the live path for now. Details: .agents/specs/parser-assembly-c8.md |
...
| Multimodal: image to text | Correctness-complete, speed-pending; not in the OpenAI server yet | Strict token-exact 32/32 vs vLLM 0.25.0 on Qwen3-VL-4B and Qwen3.6-27B (`Qwen3_5ForConditionalGeneration`); C++ image processor + vision tower + MRoPE/DeepStack backbone, single-sequence eager driver with on-GPU greedy sampling (device argmax, no host logit round-trip). See [Serving and API notes](#serving-and-api-notes) |
