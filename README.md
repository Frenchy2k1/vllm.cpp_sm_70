...
| Metal | Apple Silicon | Two models run end to end and pass correctness (OPT-125m, Qwen3-0.6B); 18 of 75 ops native, rest fall back to CPU on unified memory. Kernel work (batched command buffers, simdgroup GEMM, vectorised loads, coalesced attention) puts b=1 at 20.4 tok/s vs MLX-LM's 27.9. 255/256 Metal tokens match vLLM's own argmax. Indicative ([BENCHMARKS](docs/BENCHMARKS.md)) |
