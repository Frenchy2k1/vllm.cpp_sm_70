...
| Metal | Apple Silicon | Two models run end to end and pass correctness (OPT-125m, Qwen3-0.6B); 18 of 75 ops native, rest fall back to CPU on unified memory. Batched command buffers (1.50x), a decode GEMV (1.94x) and a simdgroup-matrix GEMM (+26%) put b=1 at 13.3 tok/s vs MLX-LM's 27.9; wider GEMM tiles measured no better. Optional MLX GEMM provider via `-DVLLM_CPP_MLX=ON`. Indicative ([BENCHMARKS](docs/BENCHMARKS.md)) |
