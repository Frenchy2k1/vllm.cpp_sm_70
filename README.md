...
| Unified streaming parser engine | Core landed, gated, not yet the live path | The vLLM 0.26 declarative `parser/engine/` (one shared streaming state machine: token-ID scanner, prefix-buffering lexer, JSON-argument hold-back) with qwen3/seed_oss/kimi_k2 configs and a registry, gated event-for-event vs vLLM 0.26. Additive and opt-in; the per-family parsers above stay the live path for now. Details: .agents/specs/streaming-parser-engine.md |
