...
| Metal | Apple Silicon | Two models run end to end and pass correctness (OPT-125m, Qwen3-0.6B); 18 of 75 ops native, rest fall back to CPU on unified memory. Batched command buffers, a decode GEMV, a simdgroup-matrix GEMM and typed loads put b=1 at 19.1 tok/s vs MLX-LM's 27.9; the decode GEMV runs at 81% of memory-bandwidth peak. Every Metal token now matches vLLM's own argmax exactly (256/256). Indicative ([BENCHMARKS](docs/BENCHMARKS.md)) |
