...
- **Speculative decoding** is not user-visible yet. Its foundation, the Qwen3.5/3.6 MTP draft head, is implemented and (2026-07-24) verified against the vLLM oracle on both gate checkpoints: fed vLLM's own draft inputs, our standalone head proposes the same token at every position whose reference top-1 is unambiguous (26 of 26 per checkpoint; the single remaining position per checkpoint is an exact tie in vLLM's own logits, where vLLM's own two APIs disagree with each other), with logits inside the same tolerance the whole-model gates use. The host-side scheduler and engine plumbing (draft-token scheduling, rejection rollback, and the drafter feedback loop) is also in place. The greedy rejection sampler (accept a draft token only when it matches the target's own choice, otherwise fall back to the target's token) and the per-request logits expansion are also implemented, with the CUDA path verified bit-exactly against a CPU reference at the gate models' full 248320-token vocabulary (accepted ids and accepted counts, exact equality) and clean under compute-sanitizer. The GDN speculative state path is now implemented too, which is the piece both gate checkpoints need (they are GDN hybrids: 48 of 64 layers on the 27B, 30 of 40 on the 35B). Draft tokens no longer fall off the recurrent-state path: a request carrying drafts becomes a multi-token row with its own k+1 state slots, the recurrence writes a state snapshot after every draft position, and the next step picks its starting slot by how many tokens were accepted, so a rejection needs no undo. That equivalence is proven, not assumed: for every possible rejection point the surviving state is bit-identical to running only the accepted tokens through the ordinary decode kernel, at the real 27B and 35B GDN dimensions, on both CPU and GPU. The short causal convolution before it rewinds the same way, by exactly the accepted count. The GDN decoder layer is now wired to that path: when a step carries draft tokens the layer routes them through the speculative recurrence and convolution, and the runner uploads the per-step speculative state tensors the layer needs. That layer wiring is proven bit-for-bit against the same underlying kernels at the real 27B and 35B dimensions. The drafter's input preparation is now in place too: after a verify step, each request's input tokens are shifted left by one within its span and the just-sampled next token is spliced into the freed slot, the query length is reduced by the number of rejected tokens, and the last-token index and attention metadata the draft forward consumes are produced, unit-tested across perfect-accept, early-mismatch, single-token, multi-request, and chunked-prefill cases. The draft head's actual forward is now the paged one the speculative loop runs: the MTP head attends over its own dedicated paged KV-cache layer using the target's block table and slot mapping, and a callable k=1 propose (draft input-prep, one paged forward, argmax draft pick) is assembled and tested. That paged forward reproduces the standalone head's logits and argmax on both gate checkpoints (the paged rewrite did not change the head math), and a two-step drive proves the head reads back the K/V it wrote a step earlier. The registry and forward seam the runner reaches the model through was fully type-erased, so it could not reach the concrete target weights, the head's hidden-state input, or the loaded draft weights the loop needs; those typed access paths are now added (a hidden-state tap on the forward, a draft-construction hook on the loaded model, draft-weight loading behind the speculative config, and a widened runner constructor), all additive and inert when speculative decoding is off, along with a latent grouping fix so the draft's own attention cache cannot be mistaken for the target's. The `--speculative-config '{"method":"mtp","num_speculative_tokens":1}'` flag and the verify/propose runner loop are now wired end to end: the engine parses the flag, builds the widened speculative KV cache and the draft, forces synchronous scheduling, and each step captures the target's hidden state, verifies the drafted tokens with the greedy rejection sampler, and proposes the next ones out of band for the following step. The speculative decode loop now runs end to end on the gate models. The ordinary convolution and state-gather kernels were made aware of the wider per-request speculative buffer (they operate on the leading ordinary window of a widened row, so a model without speculation is byte-for-byte unchanged), and a subtle input-preparation bug that overwrote each drafted token with the already-committed one (so nothing was ever accepted) was found and fixed. With those in place the single-request greedy path is correct and measured: on the 27B, our speculative-on greedy output is token-for-token identical to both our own speculative-off output and vLLM's own MTP speculative greedy output, and the drafter is genuinely working (on a short, highly predictable factual prompt every one of its 16 drafts was accepted, roughly halving the number of full model steps). It behaves exactly as before with no speculative config, byte for byte (the draft cache is not allocated and the head's forward is never called), proven by the 27B, 35B, and Coder greedy gates still passing token-exact with the speculative code present. That single-request path is now also measured for speed against vLLM running the same speculative config, on the 27B at concurrency 1 (with an example-only benchmark flag; the engine itself is unchanged): our speculative-on decode is at or above vLLM's speculative-on decode on every axis measured. Time per output token is 66.2 ms (prose) and 62.95 ms (code) versus vLLM's 69.1 and 65.3 ms (about 1.04x faster), decode throughput is 15.1 and 15.7 tokens/s versus 14.4 and 15.1 (about 4-5% higher), inter-token latency and time to first token are also lower, and the drafter acceptance rate (ours 0.85 prose, 0.92 code) is within measurement noise of vLLM's (0.84 overall). Speculative decoding helps both engines about 1.5 to 1.6x at this operating point, and our engine is already about 4% faster than vLLM with speculation off, so the lead is preserved with it on. The extra state that speculation needs (a doubled recurrent-state slot at k=1, plus the draft cache and head) costs about 3.6 GB, well inside the box's unified memory. What remains before it is a general, user-facing feature: the multi-request / concurrent path and a supported speculative flag on the OpenAI server. Both are now in place. Speculative decoding is enabled with `--speculative-config '{"method":"mtp","num_speculative_tokens":1}'` on the OpenAI server (and the example CLI, and the C API), unset by default and byte-identical to the non-speculative engine when unset. The concurrent (multi-request) path, where a speculative-decoding request shares a scheduler step with an ordinary prefill, is implemented: the linear-attention (GDN) layer splits the batch into its speculative and ordinary rows, runs each through its own recurrence over disjoint per-request state, and merges the results back. That split-merge is proven correct independently of the model: because the two groups use disjoint state, the mixed batch's output equals the speculative request run alone followed by the ordinary request run alone, bit-for-bit, at the real 27B and 35B dimensions. At higher concurrency speculative decoding keeps helping on the 27B: our speculative-on decode throughput is about 1.5x our speculative-off at concurrency 2, 4, and 8 (prose and code), and time per output token stays around 67 to 77 ms with speculation on versus 105 to 113 ms off, so it does not go neutral as the batch fills. One honest caveat at concurrency above one: the 27B's greedy output is not bit-stable across batch shapes (changing the batch size flips a few near-tie tokens even with speculation off), so exact token-for-token agreement between speculative-on and speculative-off, which holds at concurrency 1, is not attainable at higher concurrency for this model; correctness there rests on the model-independent bit-exact proof above and on matching drafter acceptance, not on identical token streams. The next speculative method, DFlash (block-diffusion drafting, which drafts a whole block of tokens at once from a small separate draft model), is scoped and dispatch-ready but not yet implemented: its design reuses the MTP machinery above, and the draft checkpoints exist and fit the hardware, but no DFlash code is present yet.
