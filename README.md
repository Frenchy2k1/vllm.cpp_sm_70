...
| Metal | Apple Silicon | Two models run end to end and pass correctness (OPT-125m, Qwen3-0.6B); 18 of 75 ops native, rest fall back to CPU on unified memory. Kernel work (batched command buffers, simdgroup GEMM, vectorised loads, coalesced attention) puts b=1 decode at 26.6 tok/s against MLX-LM's 27.9 (95.4%). Query-tiled prefill attention lifts a 4096-token prompt 32%. Indicative ([BENCHMARKS](docs/BENCHMARKS.md)) |
