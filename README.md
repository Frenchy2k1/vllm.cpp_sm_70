...
Speculative decoding (MTP, k=1) is available on the Qwen3.5/3.6 checkpoints via `--speculative-config`, at concurrency 1 (token-exact and faster than vLLM) and above (mixed-batch concurrency, on par or above vLLM). It is end-to-end token-exact vs vLLM 0.25.0 on BOTH gate models: the 27B GDN hybrid (`Qwen3_5MTP`) and the 35B MoE (`Qwen3_5MoeMTP`), each three-way identical (our spec-ON == our spec-OFF == vLLM `--speculative-config mtp` greedy) with the draft head alive (acceptance matched to vLLM). Block-diffusion DFlash speculative decoding is now unblocked (the pin advanced to vLLM 0.26.0.dev0, which resolves vllm#40898) and its first two bricks landed (2026-07-26): the DFlash oracle golden is captured on the new stack with the z-lab Qwen3.6-27B draft genuinely alive (greedy acceptance 2.2 to 8.8 accepted tokens per draft block on the fixed prompt battery), and the multi-layer auxiliary hidden-state tap the drafter conditions on is implemented in the target forward (config-gated, byte-identical when off, with the 27B MTP and text gates re-passing unchanged). The DFlash drafter model and its non-causal in-block attention have now landed and passed their GPU gate on the GB10 (2026-07-26): the draft model, the project's first bidirectional (non-causal) in-block attention primitive, the auxiliary-feature combine, and the checkpoint loader are implemented, with the causal attention every other model uses left byte-identical. The draft forward is verified against a reference dumped from vLLM's own loaded DFlash draft: the feature combine and every per-layer hidden state match to well inside the bf16 envelope (0.5 to 1.3 percent relative), and the proposed token ids match vLLM on the deterministic rows with the remainder resolving to vLLM's own bf16 near-ties. The new bidirectional kernel matches its CPU reference to the f32 softmax envelope and is memory-clean under compute-sanitizer, and the 27B text and MTP gates re-pass byte-identical. The context-KV precompute (projecting the target features to the draft's per-layer key/value cache with a bulk k-norm and RoPE) and prepare_dflash_inputs (the mask-block, context-slot, and per-mask sample-map builder) have since landed CPU-gated (2026-07-26): the input builder is integer bit-exact to vLLM's kernel (proven RED-first against a wrong rejected-token exclusion) and the context-aware draft forward degenerates exactly to the context-free forward when the context is empty, reusing the existing bidirectional attention with no new kernel. These now pass their GPU numeric parity on the GB10 (2026-07-26) against a reference dumped from vLLM's own loaded DFlash draft: the input builder is integer bit-exact to vLLM's actual kernel, the precomputed context key/value states agree to about 0.3 percent relative across all draft layers, and the block proposal matches vLLM on the deterministic rows with the rest in vLLM's own bf16 near-tie clusters, with the 27B text and MTP gates re-passing byte-identical. The DFlash propose brick (the non-autoregressive whole-block propose that composes the context-aware draft forward with a greedy per-mask-position pick) and `--speculative-config` acceptance of `method: "dflash"` have since landed CPU-gated (2026-07-26), config-gated so the MTP and non-speculative paths stay byte-identical. The engine-loop integration has since landed and run end to end on the GB10 (2026-07-26): the runner loads the separate z-lab draft alongside the target (sharing the target's bf16 embedding and output projection), captures the multi-tap on each verify forward, accumulates each request's combined-feature context across steps (honoring the rejected-token rollback), and runs the whole-block propose. On the four fixed golden prompts our DFlash-on greedy output is token-exact to vLLM's DFlash-on output on two (the code and robotics prompts) and diverges on the other two at a single bf16 near-tie each (one of which re-converges one token later), with drafter acceptance matching vLLM on all four (accepted 19, 39, 29, 25 versus vLLM 17, 39, 30, 25). The two divergences are the ratified near-tie root the gate form anticipated (the roughly one percent bf16 envelope of the inline context-KV recompute versus vLLM's fused paged draft cache), not a wiring bug, proven by the two exact prompts, the near-exact acceptance, and the 27B text and MTP gates re-passing byte-identical. A strict four-of-four token identity and the speed A/B are the remaining DFlash step (a persistent paged draft cache that bit-matches vLLM's fused projections, plus the uniform-block graph), so DFlash is not yet a supported user-facing mode. The gate form is not the MTP three-way identity: vLLM's own DFlash greedy output differs from its non-speculative greedy at k=16 near-ties, so DFlash is gated as a strict, run-deterministic match to vLLM's DFlash-on output. Not yet supported: multimodal / vision, LoRA, multi-GPU, and the full tool-calling template surface. See [Serving and API notes](#serving-and-api-notes).
...
- **Speculative decoding (MTP, k=1)** is shipped and user-facing on the Qwen3.5/3.6 gate checkpoints, enabled with `--speculative-config '{"method":"mtp","num_speculative_tokens":1}'` on the OpenAI server, the example CLI, and the C API (ABI v6 `speculative_config`). Unset by default and byte-identical to the non-speculative engine when unset (the 27B, 35B, and Coder greedy gates still pass token-exact with the speculative code present). At concurrency 1 our speculative-on greedy output is token-for-token identical to both our own speculative-off output and vLLM's own MTP speculative greedy output, and it is measured about 1.04x faster than vLLM's speculative-on decode (time per output token 66.2 ms prose / 62.95 ms code vs vLLM 69.1 / 65.3), with matching drafter acceptance. The concurrent (multi-request) path, where a speculative request shares a scheduler step with an ordinary prefill, is implemented via a GDN split-merge over disjoint per-request state (proven bit-exact against the two requests run alone). Honest caveats: only MTP at k=1, only the Qwen3.5/3.6 checkpoints that ship an `mtp.*` head (safetensors), and at concurrency above 1 the 27B's greedy output is not bit-stable across batch shapes, so exact token agreement between spec-on and spec-off is a concurrency-1 property only (correctness above that rests on the model-independent bit-exact GDN proof and on matching acceptance, not identical token streams). The block-diffusion **DFlash** speculator is unblocked (the pin advanced to vLLM 0.26.0.dev0, resolving vllm#40898 under the V2 model runner) and its first two bricks are landed, though the drafter model is not yet built so DFlash is not user-facing. The DFlash oracle golden is captured on the new stack with the z-lab Qwen3.6-27B draft alive (greedy acceptance 2.2 to 8.8 accepted tokens per k=16 draft block across a prose and code battery), which gives the reference a port is verified against. The multi-layer auxiliary hidden-state tap the drafter conditions on is implemented in the Qwen3.5/3.6 target forward, config-gated and byte-identical when off (the 27B MTP end-to-end and 27B text SACRED gates re-pass unchanged on the new oracle, with a zero-error compute-sanitizer run on the tap). The DFlash drafter model, its non-causal in-block attention (the project's first bidirectional block attention), the auxiliary-feature combine, and the checkpoint loader are now implemented and passed their GPU gate on the GB10 (2026-07-26): the bidirectional block attention is a new, separate operator, so the causal attention every other model uses is byte-identical, proven by the existing attention and dense-forward tests re-passing unchanged. The new kernel builds warning-clean under the CUDA strict-warning flags, matches its CPU reference across all five masking cases to the f32 softmax envelope, and is memory-clean under compute-sanitizer. The draft forward is verified against a reference dumped from vLLM's own loaded DFlash draft (reached through the worker over collective RPC): the feature combine and every per-layer hidden state agree to 0.5 to 1.3 percent relative, well inside the bf16 envelope, and the proposed token ids match vLLM on the deterministic rows with the remainder falling in vLLM's own bf16 near-tie clusters. The 27B text SACRED and 27B MTP gates re-passed byte-identical alongside. The context-KV precompute (the fused projection of the target features into the draft's per-layer key/value cache, with a bulk k-norm and RoPE) and prepare_dflash_inputs (the builder for the mask-token block, the context positions and slots, and the per-mask sample maps) have since landed CPU-gated (2026-07-26): the input builder is integer bit-exact to vLLM's kernel, proven RED-first against a wrong rejected-token exclusion, and the context-aware draft forward reuses the existing bidirectional attention (no new kernel) and degenerates exactly to the context-free forward when the context is empty. These passed their GPU numeric-parity gate on the GB10 (2026-07-26) against a reference dumped from vLLM's own loaded DFlash draft: the input builder is integer bit-exact to vLLM's actual Triton kernel (the reference launches that kernel directly), the precomputed context key/value states agree to about 0.3 percent relative across all five draft layers (which also proves our per-layer key/value slicing reproduces vLLM's fused projection), and the block proposal matches vLLM on the deterministic rows with the remainder in vLLM's own bf16 near-tie clusters. The build is warning-clean under the CUDA strict-warning flags (D3 adds no kernel), and the 27B text SACRED, 27B MTP, and D2 draft-parity gates re-passed byte-identical alongside. The DFlash speculator propose brick (the non-autoregressive whole-block propose that composes the context-aware draft forward with a greedy per-mask-position argmax) and `--speculative-config` acceptance of `method: "dflash"` have since landed CPU-gated (2026-07-26): the brick reproduces a greedy argmax over the context-aware draft logits (proven RED-first against reading the anchor row) and is unreachable unless a DFlash config is set, so the MTP and non-speculative engine paths stay byte-identical. The engine-loop integration has since landed and run end to end on the GB10 (2026-07-26): the runner loads the separate z-lab draft alongside the target (sharing the target's bf16 embedding and output projection), captures the multi-tap on each verify forward, accumulates each request's combined-feature context across steps with the rejected-token rollback, and runs the whole-block propose. On the four fixed golden prompts our DFlash-on greedy output is token-exact to vLLM's DFlash-on output on two, and diverges on the other two at a single bf16 near-tie each (one re-converges one token later), with drafter acceptance matching vLLM on all four (accepted 19, 39, 29, 25 versus vLLM 17, 39, 30, 25). Those two divergences are the ratified near-tie root, the roughly one percent bf16 envelope of the inline context-KV recompute versus vLLM's fused paged draft cache, not a wiring bug (the 27B text and MTP gates re-pass byte-identical). Still to build before DFlash is a supported user-facing mode: a strict four-of-four token identity (a persistent paged draft cache that bit-matches vLLM's fused projections) plus the uniform-block graph and the speed A/B. One honest correctness note surfaced by the golden: vLLM's own DFlash greedy output is not token-identical to its non-speculative greedy output (the k=16 block verify diverges from sequential decode at bf16 near-ties), so DFlash is gated as a strict, run-deterministic match to vLLM's DFlash-on output rather than the MTP three-way identity. Usage guide: [docs/SPECULATIVE-DECODING.md](docs/SPECULATIVE-DECODING.md); measured A/B in [docs/BENCHMARKS.md](docs/BENCHMARKS.md).
