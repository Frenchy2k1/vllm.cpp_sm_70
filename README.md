...
| Sampling | Supported | Greedy, temperature, top-k, top-p, min-p, presence/frequency/repetition penalties, seed, stop / stop_token_ids, min_tokens, logit_bias, allowed_token_ids, bad_words, honored from the OpenAI request and applied in vLLM's exact order. Sample logprobs are produced by the sampler; the OpenAI logprobs payload is not yet emitted |
