...
- **Multimodal / vision, LoRA, and multi-GPU** are not supported yet. Multimodal (image/video/audio) is now the top roadmap priority and is scoped: the two gate models are already multimodal architectures running text-only, so the track completes them, starting with image on Qwen3-VL-4B then Qwen3.6 (design in [.agents/specs/multimodal-track.md](.agents/specs/multimodal-track.md)).
