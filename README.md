...
| Unified streaming parser engine | Core, assembly, serving-SSE dispatch landed, gated; 8 of 10 engine-backed families ported | The vLLM 0.26 declarative `parser/engine/` (shared state machine plus 8 configs: qwen3, seed_oss, kimi_k2, minimax_m2, glm47_moe, deepseek_v4/v32, nemotron_v3) and assembly layer, gated field-for-field vs vLLM 0.26. An engine-backed `--tool-call-parser` name drives the live chat SSE chunks, off by default. gemma4 and inkling deferred. Details: .agents/specs/parser-assembly-c8.md |
...
| Multimodal: audio to text | Correctness-complete, speed-pending; not in the OpenAI server yet | End-to-end on Voxtral-Mini-3B (near-tie-robust, decoder proven token-exact 48/48); Whisper-class encoder tower (203/203 faithful) plus adapter into the landed Mistral decoder; first audio understanding in the tree. The audio decode step is now graph-captured, a small real win (60.9 vs 61.7 ms per token) that narrows the gap to vLLM to about 1.49x |
