// vllm.cpp original (MiniCPM W4 — THE SACRED correctness gate for the first
// OpenBMB MiniCPM model); no upstream mirror (upstream's MiniCPM coverage is the
// generic text-generation entry — spec §"Tests to port").
//
// THE PAGED-ENGINE MiniCPMForCausalLM (MiniCPM-2B-sft-bf16, bf16) GREEDY
...
// against the pinned vLLM 0.25.0 oracle. This proves the THREE scalar deltas
// (scale_emb / scale_depth / dim_model_base) are wired correctly — a mis-set
// scalar or dropped scale_depth emits FLUENT WRONG tokens far outside the near-tie
// band (the OPT silent-corruption mode).
//
// GATE FORM (selected BY MEASUREMENT, near-tie-distributional methodology): run
// vLLM's OWN per-prompt greedy K>=3 first (glm4-oracle-capture.py --per-prompt
// --trust-remote-code --model openbmb/MiniCPM-2B-sft-bf16). STRICT where vLLM is
// deterministic; the near-tie band is the honest fallback where vLLM's own bf16
// prefill/decode disagree on a tie. A token OUTSIDE vLLM's teacher-forced band (gap
// > kNearTieMnats on OUR prefix) is a REAL forward divergence the gate FAILS on.
//
// NOTE (vehicle): MiniCPM-2B ships ONLY pytorch_model.bin (pickle), no safetensors,
// and no native transformers/vLLM config (trust_remote_code). The gate vehicle is
// the OFFICIAL openbmb weights converted .bin->safetensors via trusted torch (same
// weights); BOTH the vLLM oracle golden and our engine read that identical
// safetensors, so the gate compares like-for-like. See the batch spec §0.3 W0.
//
// Goldens (tests/parity/goldens/minicpm_greedy_2b/, captured on dgx):
//   greedy_ids.npy / greedy_dist.npy / our_ids.npy / neartie_gap_mnats.npy.
// Checkpoint-GATED + dgx-only: resolves ~/.cache/huggingface/hub/
// models--openbmb--MiniCPM-2B-sft-bf16. On CPU/CI it emits a loud SKIP.
...
// MiniCPM-2B's tokenizer.json carries a normalizer of type Sequence (NFKC-style
// prepend/replace) that our tokenizer family does not implement ("unsupported
// non-empty normalizer Sequence"), and the openbmb config has no native
// transformers/vLLM config class. To gate the FORWARD (the three scale_emb/
// scale_depth/dim_model_base scalars — the whole point of this bring-up)
// independently of that tokenizer, we feed the paged engine the ORACLE's exact
// prompt token ids (p{i}_prompt.i32, written by the oracle capture) via the
// TokensPrompt engine path (InternLM2/Command-R precedent), then compare the greedy
// continuation to vLLM's golden. Prefill + decode thus run on identical inputs to
// the oracle, so a token match proves the scalars; the string tokenizer is a
// separate, orthogonal port (follow-up: add the normalizer Sequence support).
std::vector<int32_t> LoadPromptIds(const fs::path& gdir, int64_t i) {
  const fs::path p = gdir / ("p" + std::to_string(i) + "_prompt.i32");
  std::FILE* f = std::fopen(p.string().c_str(), "rb");
  REQUIRE_MESSAGE(f != nullptr, "minicpm: prompt ids file absent: " << p.string());
...
void RunGate(const std::string& repo_dir, const std::string& golden_subdir,
  ...
  if (!fs::exists(gdir / "greedy_ids.npy")) {
    MESSAGE(label << " greedy golden absent; skipping — capture on dgx: "
            "glm4-oracle-capture.py --per-prompt --trust-remote-code --model "
  ...
  if (!have_gap) {
    MESSAGE(label << " gap goldens absent; skipping — run under VT_DUMP_IDS=1 "
            "then glm4-neartie-gap.py --model openbmb/MiniCPM-2B-sft-bf16");
    return;
  }
...
// MiniCPM-2B-sft-bf16 (dense, Llama + 3 scalar deltas) — the first OpenBMB MiniCPM
// SACRED gate. Proves scale_emb / scale_depth / dim_model_base are applied.
TEST_CASE("minicpm-2b dense paged-engine greedy correctness gate (dgx-only, SACRED)") {
  RunGate("models--openbmb--MiniCPM-2B-sft-bf16", "minicpm_greedy_2b", "minicpm-2b");
}
