// vllm.cpp original (StableLM W4 — THE SACRED correctness gate for the first
// Stability AI StableLM model); no upstream mirror (upstream's StableLM coverage
// is the generic text-generation entry — spec §"Tests to port").
//
// THE PAGED-ENGINE StableLmForCausalLM (stablelm-2-1_6b, bf16) GREEDY CORRECTNESS
...
// vLLM 0.25.0 oracle. This proves the ZERO-NEW-KERNEL deltas are wired correctly:
// nn.LayerNorm (weight+bias, NON-fused residual — not RMSNorm), the partial NeoX
// RoPE fraction (rotary_dim 16 of head_dim 64, partial_rotary_factor 0.25), and
// the merged qkv BIAS (use_qkv_bias). A wrong rotary fraction, a missing qkv bias,
// or RMSNorm-instead-of-LayerNorm emits FLUENT WRONG tokens far outside the
// near-tie band (the OPT silent-corruption mode).
//
...
// exactly like Granite-3. (W0's BATCHED 24-token run showed one bf16 near-tie flip
// on "Once upon a time,", so the ratified near-tie band is retained as the honest
// safety net for the residual ties our decoder resolves differently from vLLM's.)
// A token OUTSIDE vLLM's teacher-forced band (gap > kNearTieMnats on OUR prefix) is
// a REAL forward divergence the gate FAILS on; strict token-exact prompts are
// counted and reported separately.
//
// Goldens (tests/parity/goldens/stablelm_greedy_1_6b/, captured on dgx):
//   greedy_ids.npy / greedy_dist.npy / our_ids.npy / neartie_gap_mnats.npy.
// Checkpoint-GATED + dgx-only: resolves ~/.cache/huggingface/hub/
// models--stabilityai--stablelm-2-1_6b. On CPU/CI it emits a loud SKIP.
...
void RunGate(const std::string& repo_dir, const std::string& golden_subdir,
  ...
  if (!have_gap) {
    MESSAGE(label << " gap goldens absent; skipping — run under VT_DUMP_IDS=1 "
            "then glm4-neartie-gap.py --model stabilityai/stablelm-2-1_6b");
    return;
  }
...
// stablelm-2-1_6b (dense, nn.LayerNorm + partial NeoX RoPE + qkv bias + SwiGLU) —
// the first Stability AI StableLM SACRED gate. Proves the LayerNorm block, the
// partial-rotary fraction, and the merged qkv bias are wired correctly.
TEST_CASE("stablelm-2-1_6b dense paged-engine greedy correctness gate (dgx-only, SACRED)") {
  RunGate("models--stabilityai--stablelm-2-1_6b", "stablelm_greedy_1_6b",
          "stablelm-2-1_6b");
}
