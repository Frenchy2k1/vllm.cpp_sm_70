// vllm.cpp original (MiniCPM3 W4 — THE SACRED correctness gate for the first
// MLA-attention MiniCPM model); no upstream mirror (upstream's MiniCPM3 coverage
// is the generic text-generation entry — spec §"Tests to port").
//
// THE PAGED-ENGINE MiniCPM3ForCausalLM (MiniCPM3-4B, bf16) GREEDY CORRECTNESS
// GATE. Drives the standard prompt battery through the FULL PAGED LLMEngine stack
// via LoadedEngine::FromModelDir and checks the greedy decode against the pinned
// vLLM 0.25.0 oracle. This proves BOTH the three MiniCPM scalar deltas
// (scale_emb / scale_depth / dim_model_base) AND the MLA re-wiring (the landed
// DeepSeek-V2 MLA block threaded with MiniCPM3 dims + is_neox_style=True LongRoPE):
// a swapped nope/rope split, a dropped kv_a_layernorm, or a mis-set scalar emits
// FLUENT WRONG tokens far outside the near-tie band.
//
...
// --trust-remote-code --model <converted MiniCPM3-4B>). STRICT where vLLM is
// deterministic; the near-tie band is the honest fallback where vLLM's own bf16
// prefill/decode disagree on a tie. A token OUTSIDE vLLM's teacher-forced band
// (gap > kNearTieMnats on OUR prefix) is a REAL forward divergence the gate FAILS.
//
// NOTE (vehicle): MiniCPM3-4B ships ONLY pytorch_model.bin (pickle), no
// safetensors, and no native transformers/vLLM config (trust_remote_code). The
// gate vehicle is the OFFICIAL openbmb weights converted .bin->safetensors via
// trusted torch (same weights, scripts/minicpm3-convert-safetensors.py); BOTH the
// vLLM oracle golden and our engine read that identical safetensors. Prompt ids
// are fed via the TokensPrompt engine path (InternLM2/Command-R/MiniCPM precedent)
// so the forward is gated independently of the tokenizer string path.
//
// Goldens (tests/parity/goldens/minicpm3_greedy_4b/, captured on dgx):
//   greedy_ids.npy / greedy_dist.npy / our_ids.npy / neartie_gap_mnats.npy +
//   p{i}_prompt.i32.
// Checkpoint-GATED + dgx-only: resolves ~/.cache/huggingface/hub/
// models--openbmb--MiniCPM3-4B. On CPU/CI it emits a loud SKIP.
...
std::string FindSnapshot(const std::string& repo_dir) {
  ...
  // Fall back to any snapshot with a config (conversion may not have run yet).
  for (const auto& e : fs::directory_iterator(snaps, ec)) {
...
std::vector<int32_t> LoadPromptIds(const fs::path& gdir, int64_t i) {
  ...
  REQUIRE_MESSAGE(f != nullptr, "minicpm3: prompt ids file absent: " << p.string());
...
void RunGate(const std::string& repo_dir, const std::string& golden_subdir,
  ...
  if (!fs::exists(fs::path(snap) / "model.safetensors")) {
    MESSAGE(label << " model.safetensors absent; skipping — run "
            "scripts/minicpm3-convert-safetensors.py on dgx first");
    return;
  }
  ...
  if (!have_gap) {
    MESSAGE(label << " gap goldens absent; skipping — run under VT_DUMP_IDS=1 "
            "then glm4-neartie-gap.py --model <converted MiniCPM3-4B>");
    return;
  }
...
// MiniCPM3-4B (dense + MLA attention, MiniCPM scalars) — the first MLA-attention
// MiniCPM SACRED gate. Proves the MLA re-wiring + scale_emb/scale_depth/
// dim_model_base.
TEST_CASE("minicpm3-4b MLA paged-engine greedy correctness gate (dgx-only, SACRED)") {
  RunGate("models--openbmb--MiniCPM3-4B", "minicpm3_greedy_4b", "minicpm3-4b");
}
