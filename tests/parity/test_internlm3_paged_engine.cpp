// vllm.cpp original (InternLM3 W0-W4 — THE SACRED correctness gate for the
// InternLM3 alias); no upstream mirror (upstream's InternLM3 coverage is the
// generic text-generation entry — spec §"Tests to port").
//
// THE PAGED-ENGINE InternLM3ForCausalLM (internlm3-8b-instruct, bf16) GREEDY
// CORRECTNESS GATE. vLLM 0.25.0 maps the "InternLM3ForCausalLM" arch string onto
// ("llama", "LlamaForCausalLM") (registry.py:134) — InternLM3 is a PLAIN Llama
// arch (RMSNorm + NeoX + GQA(kv=2) + SiLU-SwiGLU + dynamic-NTK rope factor 6.0,
// no biases, untied lm_head), NOT InternLM2 (which needs the fused-wqkv
// interleaved split). Our engine mirrors that: a single registry ALIAS line binds
// "InternLM3ForCausalLM" to the LANDED Llama factory (kLlamaFactory) VERBATIM —
// ZERO forward/loader delta. This gate PROVES the alias runs the internlm3
// checkpoint token-for-token against the oracle (a wrong forward/rope would emit
// tokens outside the near-tie band).
//
// GATE FORM (selected BY MEASUREMENT, near-tie-distributional methodology): run
// vLLM's OWN per-prompt greedy K>=3 first (internlm2-oracle-capture.py --per-prompt
// --model internlm/internlm3-8b-instruct --out-dir <goldens>/internlm3_greedy_8b).
// STRICT where vLLM is deterministic; the near-tie band is the honest fallback
// where vLLM's own bf16 prefill/decode disagree on a tie. A token OUTSIDE vLLM's
// teacher-forced band (gap > kNearTieMnats on OUR prefix) is a REAL forward
// divergence the gate FAILS on.
//
// Goldens (tests/parity/goldens/internlm3_greedy_8b/, captured on dgx):
//   greedy_ids.npy / greedy_dist.npy / our_ids.npy / neartie_gap_mnats.npy.
// Checkpoint-GATED + dgx-only: resolves ~/.cache/huggingface/hub/
// models--internlm--internlm3-8b-instruct. On CPU/CI it emits a loud SKIP.
...
// InternLM3 ships InternLM2's NON-STANDARD fast tokenizer (no pre_tokenizer,
// fuse_unk drops spaces) that our tokenizer family does not implement. To gate the
// FORWARD (the whole point of the alias bring-up) independently of that tokenizer,
// we feed the paged engine the ORACLE's exact prompt token ids (p{i}_prompt.i32,
// written by the oracle-capture script) via the TokensPrompt engine path, then
// compare the greedy continuation to vLLM's golden. Prefill + decode thus run on
// identical inputs to the oracle, so a token match proves the forward; the string
// tokenizer is a separate, orthogonal port.
std::vector<int32_t> LoadPromptIds(const fs::path& gdir, int64_t i) {
  const fs::path p = gdir / ("p" + std::to_string(i) + "_prompt.i32");
  std::FILE* f = std::fopen(p.string().c_str(), "rb");
  REQUIRE_MESSAGE(f != nullptr, "internlm3: prompt ids file absent: " << p.string());
...
void RunGate(const std::string& repo_dir, const std::string& golden_subdir,
  ...
  if (!fs::exists(gdir / "greedy_ids.npy")) {
    MESSAGE(label << " greedy golden absent; skipping — capture on dgx: "
            ...
            "internlm/internlm3-8b-instruct --out-dir <goldens>/internlm3_greedy_8b");
    return;
  }
  if (dump && !have_gap) {
  ...
  if (!have_gap) {
    MESSAGE(label << " gap goldens absent; skipping — run under VT_DUMP_IDS=1 "
            "then internlm2-neartie-gap.py --model internlm/internlm3-8b-instruct "
            "--golden-dir <goldens>/internlm3_greedy_8b");
    return;
  }
...
// internlm3-8b-instruct (dense, PLAIN Llama forward via the "InternLM3ForCausalLM"
// registry alias) — the InternLM3 SACRED gate. Proves the alias runs the checkpoint
// token-for-token against the vLLM 0.25.0 oracle (which itself maps InternLM3 onto
// LlamaForCausalLM).
TEST_CASE("internlm3-8b-instruct dense paged-engine greedy correctness gate (dgx-only, SACRED)") {
  RunGate("models--internlm--internlm3-8b-instruct", "internlm3_greedy_8b",
