// vllm.cpp original (InternLM2 W4 — THE SACRED correctness gate for the first
// InternLM2 model); no upstream mirror (upstream's InternLM2 coverage is the
// generic text-generation entry — spec §"Tests to port").
//
// THE PAGED-ENGINE InternLM2ForCausalLM (internlm2-chat-1_8b, bf16) GREEDY
...
// against the pinned vLLM 0.25.0 oracle. This proves the fused `wqkv` INTERLEAVED
// de-split is correct — a plain [q|k|v] concat split (the layout every other
// dense family uses) SCRAMBLES query/key/value heads and emits tokens far outside
// the near-tie band, the ROOT divergence this gate catches. (The forward reuses
// the landed Llama/Qwen3-dense path VERBATIM: RMSNorm + NeoX rope + GQA +
// SiLU-SwiGLU, ZERO new kernel — so a green gate isolates the loader delta.)
//
// GATE FORM (selected BY MEASUREMENT, near-tie-distributional methodology): run
// vLLM's OWN per-prompt greedy K=5 first (internlm2-oracle-capture.py --per-prompt
// --model internlm/internlm2-chat-1_8b). STRICT where vLLM is deterministic;
...
//
// Goldens (tests/parity/goldens/internlm2_greedy_1_8b/, captured on dgx):
//   greedy_ids.npy / greedy_dist.npy / our_ids.npy / neartie_gap_mnats.npy.
// Checkpoint-GATED + dgx-only: resolves ~/.cache/huggingface/hub/
// models--internlm--internlm2-chat-1_8b. On CPU/CI it emits a loud SKIP.
...
// InternLM2's fast tokenizer is a NON-STANDARD BPE (no pre_tokenizer,
// byte_fallback=false + fuse_unk=true, so literal spaces are DROPPED) that our
// tokenizer family does not implement. To gate the FORWARD (the fused-`wqkv`
// interleaved de-split — the whole point of this bring-up) independently of that
// tokenizer, we feed the paged engine the ORACLE's exact prompt token ids
// (p{i}_prompt.i32, written by internlm2-oracle-capture.py) via the TokensPrompt
// engine path, then compare the greedy continuation to vLLM's golden. Prefill +
// decode thus run on identical inputs to the oracle, so a token match proves the
// de-interleave; the string tokenizer is a separate, orthogonal port.
std::vector<int32_t> LoadPromptIds(const fs::path& gdir, int64_t i) {
  const fs::path p = gdir / ("p" + std::to_string(i) + "_prompt.i32");
  std::FILE* f = std::fopen(p.string().c_str(), "rb");
  REQUIRE_MESSAGE(f != nullptr, "internlm2: prompt ids file absent: " << p.string());
...
void RunGate(const std::string& repo_dir, const std::string& golden_subdir,
  ...
  if (!have_gap) {
    MESSAGE(label << " gap goldens absent; skipping — run under VT_DUMP_IDS=1 "
            "then internlm2-neartie-gap.py --model internlm/internlm2-chat-1_8b");
    return;
  }
...
// internlm2-chat-1_8b (dense, Llama forward + fused-wqkv interleaved de-split) —
// the first InternLM2 SACRED gate. Proves the interleaved wqkv split is correct.
TEST_CASE("internlm2-chat-1_8b dense paged-engine greedy correctness gate (dgx-only, SACRED)") {
  RunGate("models--internlm--internlm2-chat-1_8b", "internlm2_greedy_1_8b",
          "internlm2-chat-1_8b");
}
