// vllm.cpp original (Phi-1/Phi-2 W4 — THE SACRED correctness gate for the OLDER
// Microsoft Phi architecture `PhiForCausalLM`, DISTINCT from the landed
// `Phi3ForCausalLM`); no upstream mirror (upstream's Phi coverage is the generic
// text-generation entry — spec §"Tests to port").
//
// THE PAGED-ENGINE PhiForCausalLM (microsoft/phi-2, bf16) GREEDY CORRECTNESS GATE.
// Drives the standard prompt battery through the FULL PAGED LLMEngine stack via
// LoadedEngine::FromModelDir and checks the greedy decode against the pinned vLLM
// 0.25.0 oracle. This proves the ZERO-NEW-KERNEL deltas are wired correctly:
// the GPT-J PARALLEL residual (ONE nn.LayerNorm feeds BOTH attn and mlp, summed
// back together), the nn.LayerNorm weight+bias, the biased q/k/v/dense projections,
// the partial NeoX RoPE fraction (rotary_dim 32 of head_dim 80,
// partial_rotary_factor 0.4), the NON-gated NewGELU MLP (fc1 -> gelu_new -> fc2),
// and the UNTIED lm_head with a per-vocab bias. A sequential-instead-of-parallel
// residual, a dropped qkv/dense bias, a full-instead-of-partial rotary, or a
// missing lm_head bias emits FLUENT WRONG tokens far outside the near-tie band
// (the OPT silent-corruption mode).
//
// GATE FORM (selected BY MEASUREMENT, near-tie-distributional methodology): in the
// GATE REGIME (per-prompt, batch=1, K=5) vLLM's OWN greedy is ALL-DETERMINISTIC on
// this model (0 multi-member cells) -> STRICT token-exact is the well-posed bar,
// exactly like Granite-3 / StableLM. The ratified near-tie band is retained as the
// honest safety net for any residual bf16 tie our decoder resolves differently from
// vLLM's; a token OUTSIDE vLLM's teacher-forced band (gap > kNearTieMnats on OUR
// prefix) is a REAL forward divergence the gate FAILS on. STRICT token-exact
// prompts are counted and reported separately.
//
// Goldens (tests/parity/goldens/phi2_greedy_2_7b/, captured on dgx):
//   greedy_ids.npy / greedy_dist.npy / our_ids.npy / neartie_gap_mnats.npy.
// Checkpoint-GATED + dgx-only: resolves ~/.cache/huggingface/hub/
// models--microsoft--phi-2. On CPU/CI it emits a loud SKIP.
...
void RunGate(const std::string& repo_dir, const std::string& golden_subdir,
  ...
  if (!fs::exists(gdir / "greedy_ids.npy")) {
    MESSAGE(label << " greedy golden absent; skipping — capture on dgx: "
            "phi2_w0_probe.py / internlm2-oracle-capture.py --model microsoft/phi-2");
    return;
  }
  if (dump && !have_gap) {
  ...
  if (!have_gap) {
    MESSAGE(label << " gap goldens absent; skipping — run under VT_DUMP_IDS=1 "
            "then internlm2-neartie-gap.py --model microsoft/phi-2");
    return;
  }
  ...
  for (int64_t i = 0; i < N; ++i) {
    ...
    const std::vector<int32_t>& got = out.outputs[0].token_ids;
    // Our engine STOPS at EOS; the committed anchor/gap goldens pad the tail with
    // -1. Compare only the real (non-padded) prefix, exactly as the gap golden was
    // built (internlm2-neartie-gap.py teacher-forces up to the first -1).
    int64_t L = T;
    for (int64_t j = 0; j < T; ++j) {
      if (od[i * T + j] < 0) { L = j; break; }
    }
    REQUIRE(static_cast<int64_t>(got.size()) >= L);
    if (dump) {
...
// microsoft/phi-2 (dense, GPT-J parallel-residual nn.LayerNorm + biased qkv/dense +
// partial NeoX RoPE + non-gated NewGELU MLP + untied biased lm_head) — the first
// Microsoft Phi-1/2 SACRED gate. Proves the parallel-residual wiring, the biases,
// the partial-rotary fraction, and the non-gated GELU MLP are wired correctly.
TEST_CASE("microsoft/phi-2 dense paged-engine greedy correctness gate (dgx-only, SACRED)") {
  RunGate("models--microsoft--phi-2", "phi2_greedy_2_7b", "microsoft/phi-2");
}
