...
bool RunQwen36Logits(Backend& /*b*/, Queue& q, const fs::path& dir,
  ...
  MESSAGE("qwen36_logits: prefill forward on T=" << T0 << " (vocab " << vocab
                                                 << ")...");
  std::vector<float> logits = forward(token_ids, positions, weights, cfg, q, nullptr);
  ...
  for (int64_t s = 0; s < n_greedy; ++s) {
    ...
    if (s + 1 < n_greedy) cur = forward(ids, pos, weights, cfg, q, nullptr);
  }
...
// Multi-device MoE token-equal gate (tp==tp1): the 35B-A3B MoE analog of the
// dense 27B gate above. The MoE per-rank expert shard (MoeBlock tp>1 ->
// vt_cuda_moe_expert_shard_run: replicated router, expert-intermediate split,
// exit-only AllReduce) rests on the invariant that a tp>1 MoE forward equals
// the tp1 forward token-for-token. Run the real 35B ForwardDense greedy twice —
// tp1 (null) and a tp thunk with world_size>1 (the collectives happen INSIDE
// vt_cuda_moe_expert_shard_run / vt_cuda_attn_kv_shard_run) — and require
// DEVICE-identical greedy tokens. Same TpThunkComm as the 27B gate.
TEST_CASE("qwen36 moe tp==tp1 token gate (dgx-only, CUDA, multi-GPU)") {
  bool has_cuda = true;
  try { vt::GetBackend(DeviceType::kCUDA); }
  catch (const std::runtime_error&) { has_cuda = false; }
  if (!has_cuda) { MESSAGE("no CUDA backend; skipping moe tp==tp1 gate"); return; }
  const std::string snap = Find35BSnapshot();
  if (snap.empty()) { MESSAGE("SKIP: 35B-A3B-NVFP4 snapshot not present"); return; }
#if VLLM_CPP_CUDA
  int ngpu = 0;
  if (cudaGetDeviceCount(&ngpu) != cudaSuccess) { MESSAGE("cudaGetDeviceCount failed"); return; }
#else
  ...
  MESSAGE("moe tp==tp1 gate: loading real 35B (NVFP4 fp4-resident) on " << ngpu << " GPUs...");
  const vllm::HfConfig cfg = vllm::LoadHfConfig((fs::path(snap) / "config.json").string());
  std::vector<vllm::SafetensorsFile> shards;
  for (int n = 1; n <= 3; ++n) {
    auto s = OpenShard(snap, n);
    if (!s) { MESSAGE("SKIP: shard " << n << " not found"); return; }
    shards.push_back(std::move(*s));
  }
  const vllm::Qwen3_5MoeWeights weights = vllm::LoadQwen3_5Moe(shards, cfg);

  // Use the pinned 35B logits golden prompt (deterministic, in-tree).
  const fs::path dir = fs::path(PARITY_GOLDENS_DIR) / "qwen36_logits_35b";
  if (!fs::exists(dir / "manifest.json")) { MESSAGE("35B logits golden absent; skipping moe tp gate"); return; }
  json m = json::parse(std::ifstream(dir / "manifest.json"));
  auto ids_l = LoadTensor(dir, m["tensors"]["token_ids"]);
  const int32_t* idp = ids_l.tensor.Ptr<int32_t>();
  std::vector<int32_t> base(idp, idp + ids_l.tensor.shape[0]);
  std::vector<int32_t> pos(base.size());
  for (size_t i = 0; i < pos.size(); ++i) pos[i] = static_cast<int32_t>(i);
  const int vocab = static_cast<int>(cfg.vocab_size);
  ...
  const auto run_greedy = [&](const vllm::TensorParallel* tp) {
    std::vector<int32_t> ids = base, ps = pos;
    std::vector<float> lg = vllm::Qwen3_5Model::ForwardDense(ids, ps, weights, cfg, q, tp);
    std::vector<int32_t> gen;
    constexpr int kSteps = 8;
    for (int s = 0; s < kSteps; ++s) {
      const int32_t nxt = ArgmaxRow(&lg[static_cast<size_t>(ids.size() - 1) * vocab], vocab);
      gen.push_back(nxt);
      ids.push_back(nxt); ps.push_back(static_cast<int32_t>(ps.size()));
      if (s + 1 < kSteps) lg = vllm::Qwen3_5Model::ForwardDense(ids, ps, weights, cfg, q, tp);
    }
    return gen;
  };
  const std::vector<int32_t> t1 = run_greedy(nullptr);
  ...
  b.DestroyQueue(q);
  REQUIRE(t1 == tw);
  MESSAGE("moe tp==tp1 token gate: " << ngpu << "-GPU MoE == tp1 (" << t1.size()
          << " tokens identical)");
}
