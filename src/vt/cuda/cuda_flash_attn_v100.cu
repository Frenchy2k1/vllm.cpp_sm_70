...
// by construction, `#error` above 7.0). The full paged-attention bodies built
// on this fragment core (decode + prefill fast paths) live in
// cuda_sm70_flash_attn.cu; the compile gate here makes a build failure if the
// vendored fragment abstraction breaks for Volta.
//
// This cell only ever compiles for sm_70 (feature `sm70-fa2-v1`); the TU's
// content is compile-time instantiations of the vendored core so a build
// fails if the fragment abstraction breaks for Volta.
