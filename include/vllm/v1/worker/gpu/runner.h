...
class GPUModelRunner final : public ModelRunnerBase {
 ...
 private:
  ...
  // Acquire the group + wrap this rank's communicator; tp<=1 / non-NCCL / non-
  // NCCL-init-failure builds no-op; a real multi-rank group is REFUSED here
  // (named exception). Safe to call once per runner (delegating ctors).
