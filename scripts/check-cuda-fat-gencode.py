...
audit a different requested set — e.g. --archs 70 for the Volta lane, whose
expected set is the portable TUs plus the sm70 feature TUs (NVFP4 GEMM, FA2
fragment core + paged decode).
"""
...
def _required_sources() -> list[str]:
    ...
    compiles. In a `--archs 70` (Volta) build the expected set is the portable
    TUs plus the sm70 feature TUs (W4A16 NVFP4 GEMM, FA2 fragment core + paged
    decode); the fp8/sm12x/cutlass/marlin feature TUs are absent by
    construction and must NOT be demanded — but if one of them nonetheless
    appears in the compile database with any gencode, the per-source check
    below rejects it (its expected set is empty for this target)."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "separated (default: the ten-SM archive). e.g. '70' audits the Volta "
        "lane (portable + the sm70 feature TUs).",
    )
