#!/usr/bin/env python3
# ADDITIVE-MODEL W4 — capture the vLLM 0.25.0 oracle greedy continuation for the
# InternLM2 (`InternLM2ForCausalLM`) checkpoint on the standard prompt battery.
# trust_remote_code=True: InternLM2 ships a custom config/tokenizer (auto_map);
# vLLM still runs its NATIVE InternLM2ForCausalLM model class, only the config +
# SentencePiece tokenizer come from the remote code.
#
...
#
# The prompt list MUST match tests/parity/test_glm4_paged_engine.cpp::Prompts()
...
#       --model zai-org/GLM-4-9B-0414 --runs 10 --out-dir <goldens>/glm4_greedy_9b
#   PATH=$HOME/venvs/vllm-oracle/bin:$PATH \
#     ~/venvs/vllm-oracle/bin/python scripts/qwen3-oracle-capture.py \
...
def default_out_dir():
    return os.path.join(
        ...
        "tests", "parity", "goldens", "internlm2_greedy_1_8b",
    )


def parse_args():
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--model", default=os.environ.get("INTERNLM2_MODEL", "internlm/internlm2-chat-1_8b"))
    ap.add_argument("--out-dir", default=None,
                    help="golden dir (default tests/parity/goldens/glm4_greedy_9b)")
    ap.add_argument("--runs", type=int, default=int(os.environ.get("INTERNLM2_RUNS", "5")),
                    help="K greedy runs to build the observed distribution (K>=1)")
    ap.add_argument("--max-tokens", type=int, default=16)
    ap.add_argument("--gpu-mem", type=float,
                    default=float(os.environ.get("INTERNLM2_GPU_MEM", "0.35")),
