...
| Dense W4A16 MLP runs ONE merged `gate_up` Marlin GEMM (vLLM's `MergedColumnParallelLinear` topology) | ◐ `VT_DENSE_MARLIN_GATEUP`, **default OFF** pending the A/B (#365). The dense 27B MLP still launches two split GEMMs: 193 Marlin calls/step vs the oracle's 129 | ✅ | ☐ | ☐ |
