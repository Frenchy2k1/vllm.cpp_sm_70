...
| `MuseGlimmerForCausalLM` | none (no checkpoint run) | W1 TEXT TOWER: forward + loader gated 16/16 vs an fp32 reference transcribed from vllm#51655, plus per-mechanism property tests; scaffold 11/11 still green. NOT token-exact vs the HF reference (W2) | not measurable; pinned oracle cannot load `muse_glimmer` |
| `MuseGlimmerForConditionalGeneration` | none (no checkpoint run) | Same W1 text tower; vision refuses (W3). Config parse gated against the RELEASED 30B config as a fixture: the iRoPE split is read from `layer_rope_theta`/`layer_types` | not measurable; anchored to open vllm#51655, pin advance owed |
...
| Muse Glimmer 30B (Meta) | Text tower lands (W1); no checkpoint run, vision pending | [spec](../.agents/specs/muse-glimmer.md) / [#268](https://github.com/mudler/vllm.cpp/issues/268). W1 gates the text forward at unit scale only; token-exact e2e is W2, vision W3. No speed claimable until the pin advances |
