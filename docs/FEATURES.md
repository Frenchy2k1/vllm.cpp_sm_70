...
| Multi-GPU execution (tensor parallel) | tp>1 math proven tp==token on 2×V100 (gate, GPU 0,1); the real per-rank server weight-load is NOT wired (a tp>1 server refuses loudly at runner construction, named missing pieces), it does not deadlock | sharded tp>1 math or the loud multi-GPU serve refusal; qwen3.5 dense+MoE shard for real |
