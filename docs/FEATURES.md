...
| Qwen3.8-27B quantized arms (Q4_K_M GGUF, its `clip` mmproj, the `unsloth` "NVFP4") | BF16 gated (#915); the `unsloth` "NVFP4" loads once the per-channel FP8 GDN loader lands (FP8 W8A8 `linear_attn`/`self_attn` per-output-channel + NVFP4 MLP); Q4_K_M GGUF + `clip` mmproj still owed | [spec](../.agents/specs/qwen38-27b-quant-arms.md) + [per-channel GDN loader](../.agents/specs/qwen38-per-channel-gdn-loader.md) |
