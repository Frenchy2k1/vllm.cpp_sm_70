...
| `MuseGlimmerForCausalLM` | Muse-Glimmer-30B real tensors, **reduced depth 4/52 only**: 5 prefill argmax positions match a torch transcription of vllm#51655 and HF's own impl. Full depth never ran | text forward + loader vs an fp32 reference from vllm#51655, per-mechanism property tests, scaffold 11/11. No generated tokens, multi-step decode untested | not measurable; pinned oracle cannot load `muse_glimmer` |
| `MuseGlimmerForConditionalGeneration` | vision: **no reference run of any kind**; enumeration gated vs the released 30B index (1436/1436) | perception encoder loaded and wired, so an image or video prompt runs. Reachability plus placeholder scatter only, no image or video correctness | not measurable; anchored to open vllm#51655 |
