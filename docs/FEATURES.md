...
| Hybrid CPU/GPU expert placement (routed-expert COMPUTE on the CPU, attention and dense on the GPU) | ◐ **Four architectures.** The four llama.cpp surfaces are mapped onto `--offload-config`'s `vllm_cpp.placement` key (`overrides`=`-ot`, `cpu_moe`=`-cmoe`, `n_cpu_moe`=`-ncmoe N`, `fit`=`--fit`), resolved to a per-layer decision at model build, and a placed layer runs its routed experts on the placement device with the hidden state crossing at the boundary. Wired: `Qwen3MoeForCausalLM`, Qwen3.5/3.6, Nemotron-H and DeepSeek-V2, all through ONE shared seam (`RunMoePlaced`), one line each. NOT wired and silently inert: **Laguna** and **DeepSeek-V4**, whose MoE entries return host `std::vector<float>` from a `vt::Queue` rather than a `DBuf` from a `Dev` and so do not fit the seam without reshaping them; and **Kimi-Linear**, whose MoE already runs on the host from host weights, where placement moves nothing by definition. The placement TARGET is the CPU (the engine may be any device); an accelerator destination is refused. The fp4-resident expert arm is refused, because its device residents are built eagerly at load so placing it would upload every expert and then compute across the bus — a defect a token gate cannot see. **NO model has been run with a placement in force**: gated by unit and round-trip tests (the round trip is byte-identical to computing in place, mutation-proven), the end-to-end token gate is owed, and the SPEED axis is blocked on a discrete rig ([#149](https://github.com/mudler/vllm.cpp/issues/149), [#2026](https://github.com/mudler/vllm.cpp/issues/2026)) | ☐ the CPU MoE kernels exist (`fused_moe/experts/cpu_moe.py`, `cpu_fused_moe.py:398,430`) and every selection site gates on `current_platform.is_cpu()` (`oracle/fp8.py:129`, `oracle/mxfp4.py:533`, `oracle/unquantized.py:97,202`), so they are reachable only when the WHOLE model runs on the CPU platform | ☐ same shape as vLLM: module-level `_is_cpu = is_cpu()` platform flags (`layers/moe/topk.py:129`, `fused_moe_triton/layer.py:83` @ `f63458b5`), no per-layer device override | ✅ `-cmoe` / `-ncmoe`, applied at load as tensor-name pattern → buffer type, so compute follows the tensor's buffer (`common/arg.cpp:2451-2478`, `common/common.h:1046-1054` @ `237ad9b96`) |
...
Per-arch build state: consumer Blackwell (`sm_120a`) and Hopper (`sm_90a`)
compile and emit real SASS but are not runtime-proven here (no such card);
Ampere/Ada (`sm_80/86/87/89`) and datacenter Blackwell (`sm_100a/103a`) are
build-verified per-arch for the fast GDN path (plus FA2 on Ampere, `sm_100a`
NVFP4 GEMM), not runtime-gated. **Volta (`sm_70`, Tesla V100) is build-verified
with portable kernels plus the `sm70-nvfp4-gemm`, `sm70-fa2-v1` and sm70 W8A16
keep-quant arms** (the `cuda-sm70-build` CI lane, CUDA 12.9.1 — nvcc 13 dropped
`sm_70` codegen), and is runtime-proven: the tp==1 and tp==2 gates ran on a
2×V100 box.
...
| Multi-GPU execution (tensor parallel) | tp>1 math proven tp==token on 2×V100; the Qwen3.8-27B NVFP4 fp8w tail-MLP layers (56–63) now run device-resident per-rank W8A16 through a real tp=2 serve, token-exact vs tp=1 with `fp8w DEVICE` markers (G5) | sharded tp>1 math and the fp8w tail-MLP per-rank serve; remaining qwen3.5 dense+MoE shard for real |
