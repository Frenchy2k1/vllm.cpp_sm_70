...
| `MuseGlimmerForCausalLM` | real tensors, **reduced depth 4/52 only**: 5 prefill argmax positions match a torch transcription of vllm#51655 and HF; full depth never ran. GGUF tokenizes (#347), forward INCOHERENT (#359) | text forward + loader vs an fp32 reference, per-mechanism property tests, scaffold 11/11, GGUF gate 12/12. No generated tokens, multi-step decode untested | not measurable in either format; pinned oracle cannot load `muse_glimmer` |
