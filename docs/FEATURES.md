...
| `MuseGlimmerForConditionalGeneration` | none (no checkpoint run) | same text tower; perception encoder gated per-stage in isolation but not wired, so image/video refuses | not measurable; anchored to open vllm#51655 |
...
| Muse Glimmer 30B (Meta) | Text tower, vision tower and parsers gated; not wired end to end | [spec](../.agents/specs/muse-glimmer.md) / [#268](https://github.com/mudler/vllm.cpp/issues/268). No checkpoint run, so no token-exact claim; no speed axis until the pin advances |
