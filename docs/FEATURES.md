...
| `MuseGlimmerForCausalLM` | none (no checkpoint run) | text forward + loader gated vs an fp32 reference transcribed from vllm#51655, per-mechanism property tests, scaffold 11/11 | not measurable; pinned oracle cannot load `muse_glimmer` |
| `MuseGlimmerForConditionalGeneration` | none (no checkpoint run); enumeration gated vs the released 30B index (1436/1436) | perception encoder now loaded and wired: an image/video prompt runs. Reachability only, no reference decode | not measurable; anchored to open vllm#51655 |
