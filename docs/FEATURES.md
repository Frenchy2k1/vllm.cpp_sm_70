...
| `MuseGlimmerForCausalLM` | no forward run; GGUF k-quant arm landed: the released `muse-glimmer-30B-kquant-17gb.gguf` loads, 731/731 tensors accounted, all 52 layers materialized | text forward + loader gated vs an fp32 reference transcribed from vllm#51655, per-mechanism property tests, scaffold 11/11, GGUF gate 12/12 (11 mutations RED) | not measurable in either weight format; the pinned oracle cannot load `muse_glimmer` |
| `MuseGlimmerForConditionalGeneration` | none (no checkpoint run); enumeration gated vs the released 30B index (1436/1436). Image/video need bf16 safetensors: the released `mmproj-kquant.gguf` is refused by name (spec §10.4) | perception encoder now loaded and wired: an image/video prompt runs. Reachability only, no reference decode | not measurable; anchored to open vllm#51655 |
...
| Muse Glimmer ATEM parsers (`muse_glimmer`) | ◐ PARSERS + FORWARD, NO REFERENCE RUN (ATEM tool dialect, `to=self` split, all five upstream parser modules ported). Both towers forward; no checkpoint run e2e ([spec](../.agents/specs/muse-glimmer.md)) | ✅ | ☐ | ☐ |
...
| Muse Glimmer 30B (Meta) | Towers and parsers gated; vision wired into the mm forward, no e2e reference run | [spec](../.agents/specs/muse-glimmer.md) / [#268](https://github.com/mudler/vllm.cpp/issues/268). No checkpoint run, so no token-exact claim for text or image; no speed axis until the pin advances |
