...
| `MuseGlimmerForCausalLM` | none (checkpoint on shared storage, not yet loaded) | scaffold: registry+config+name-map gated 11/11, forward refuses | not measurable; pinned oracle cannot load `muse_glimmer` |
| `MuseGlimmerForConditionalGeneration` | none (weights not downloaded) | scaffold gated 11/11, forward refuses; standalone perception encoder gated per-stage 7/7 vs a vllm#51655-derived torch reference (f32 rel_l2 1.4e-07), not registry-wired; image/video e2e unestablished | not measurable; anchored to open vllm#51655, pin advance owed |
...
| Muse Glimmer 30B (Meta) | Towers and parsers gated; forward refuses | Post-pin release ([spec](../.agents/specs/muse-glimmer.md), [#268](https://github.com/mudler/vllm.cpp/issues/268)). Vision tower and both parsers gated in isolation; the text forward is absent, so nothing runs end to end |
