...
**The NVFP4 modules load. The FP8 group is now implemented for the projections
that genuinely ship per-output-channel FP8**, and refused by name everywhere
else. `linear_attn.{in_proj_qkv,in_proj_z,out_proj}` and
`self_attn.{q,k,v,o}_proj` carry `F8_E4M3` weights with a `BF16 [out,1]`
per-column scale; the loader detects that exact shape+dtype pair and
dequantizes it at load (`DequantFp8ChannelToBf16`), scaling each output column
by its own channel scale. On Volta (`sm_70`) the GDN in-proj instead takes the
W8A16 keep-quant arm (raw `Fp8PerChannelWeight` bytes + f32 scale through
`kMatmulFp8W8a16`), avoiding the 2x resident-byte blow-up of the bf16 dequant.
A per-tensor `F32` scale still takes the unchanged per-tensor arm; any other
per-output-channel or block-wise FP8 checkpoint stays refused by name rather
than mis-read. The engine also refuses `kv_cache_scheme`, because it does not
consume the checkpoint's K and V scales.
...
The remaining per-channel FP8 towers (attn q/k/v/o, GDN out_proj, the tail-MLP
fp8 layers), the quantized KV cache, the resident-byte assertion, and the token
gates are still owed under
