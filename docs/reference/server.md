...
only because the capability is missing (the expert-parallel and
`--mm-encoder-tp-mode` flags), silently accepting those would let you believe
you got the parallel path when you did not. `--tensor-parallel-size` is not one
of those: it is parsed and validated, and tp>1 serving is wired and opt-in. With
more than one visible CUDA device and `VT_TP_ALLOW` unset, a tp>1 launch is
refused loudly (the G1 guard) rather than half-wired; set `VT_TP_ALLOW=1` to
serve the per-rank `tp>1` path (see [ENVIRONMENT.md](ENVIRONMENT.md)). tp1
(`CUDA_VISIBLE_DEVICES` limited to one device) is the fully-fast serve path.
