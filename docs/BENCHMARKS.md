...
- **The activation is ALREADY bf16 into the GEMV.** `LagunaGraph::GemmBf16` casts `x`(f32)→bf16 (`ab`) and calls `MatmulBT(ot_f32, ab_bf16, w_bf16)`; `MatmulBTKernelCuda` passes `CUDA_R_16BF` for both operands with `CUBLAS_COMPUTE_32F`. The `gemvx<bf16, float>` template reflects cuBLAS's **f32-OUTPUT** algo choice (it widens the bf16 vector to an f32 accumulator), not an f32 activation in memory. A prior test (`VT_LAGUNA_BF16_ACT`) already flipped the OUTPUT to bf16 and measured a **wash** — so the `<bf16,bf16>` variant is not faster here.
- **The projection GEMV is parity-by-definition.** The dominant decode cost is the bf16 projection GEMVs — measured **12.87 + 6.88 = 19.75 ms/tok (72% of the 27.3 ms/tok decode)** plus the `lm_head` GEMV 2.4 ms — the SAME cuBLAS `gemvx` vLLM calls on the same weights (`laguna-gap-is-gpu-compute-not-host`: "IDENTICAL cuBLAS gemvx = parity by definition"). Activation dtype cannot beat cuBLAS parity there.
