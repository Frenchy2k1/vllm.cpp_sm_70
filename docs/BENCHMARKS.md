...
**DeepSeek-V4-Flash W8-run — apples-to-apples ds4 benchmark (2026-07-29, `CLAIM-DEEPSEEK-V4-W8-RUN`, NOT pushed).** Disposition: **PENDING — did NOT execute (nothing faked).** The apples-to-apples plan is now fixed: the cross-engine oracle is **antirez/ds4** (DwarfStar, `make cuda-spark` on GB10) and BOTH engines run the EXACT SAME file — ds4's `q2-imatrix` GGUF (`antirez/deepseek-v4-gguf`, single 80.7 GB, IQ2_XXS gate/up + Q2_K down + Q8_0 attn/shared/out + F16 embed). Loadability into OUR engine is PROVEN (HF HTTP-range header, no download): our `blk.N.*` name-map covers ds4's file 1328/1328 exactly (0 unmapped/leftover), and three real-file loader gaps were found + fixed + gated (`compress_ratios` len 44 vs block_count; `swiglu_clamp_exp` per-layer array vs scalar — a 0 limit zeroes every expert; `ffn_gate_tid2eid` ggml I32 dequant), `test_deepseek_v4_gguf_load` **8/8·288** CPU Release `-Werror`, RED-first proven. **The benchmark numbers are NOT yet measured** — the GB10 run is blocked on: the GPU flock held by a concurrent agent, two long builds (ds4 `cuda-spark` + our aarch64 CUDA), the 80 GB download, and a greedy driver (V4's stateless keep-quant `DeepseekV4ForwardGguf` needs a manual greedy loop; the engine's incremental paged decode does not apply, and `Tokenizer::FromGguf` rejects ds4's `pre='joyai-llm'` so the token cross-check must inject ds4's ids). Honest expectation when it runs: ds4 runs the experts on the GPU while our keep-quant expert GEMMs are CPU-tier on the 20 ARM cores (no CUDA keep-quant vec_dot exists), so ds4 is expected FASTER — report the ours-vs-ds4 prefill+gen tok/s ratio with that ARM-tier attribution, plus the token cross-check (prefix agreement / first divergence) and self-consistency. Resume recipe: `flock $HOME/gpu.lock` → `docker stop local-ai-worker` (restore after) → `./download_model.sh q2-imatrix` → ds4 `make cuda-spark` + `./ds4 -p "..."` / `./ds4-bench` → our engine greedy loop over the same file (assert peak resident ≈ 81-84 GiB keep-quant, NOT f32) → cross-check + benchmark.
