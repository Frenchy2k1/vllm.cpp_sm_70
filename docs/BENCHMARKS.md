...
| Vulkan vs llama.cpp Vulkan (`BENCH-VK-LLAMA`) | 25 NATIVE (+8 GDN). **27B prefill 21.5x**, decode **4.28**/4.35. Host MEASURED ~3 ms/tok not 5.3; pipelined submit **-1.41 ms/tok, 5/5 paired**, goal NOT met; fence spin REJECTED. [Detail](../.agents/benchmark-record.md) | `VK-C` coopmat A/B on Thor (`VT_VULKAN_COOPMAT=0` A/Bs it): **11.1x-32.9x** vs our UNTILED scalar kernel, not vs a competent GEMM. `VK-E`: llama.cpp `-DGGML_VULKAN=ON` at `237ad9b96` on dgx, same GGUF, three columns |
