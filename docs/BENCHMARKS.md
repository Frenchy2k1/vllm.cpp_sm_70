...
| DSpark | Qwen3.6-35B-A3B NVFP4 + RedHatAI dspark draft, k=8 | Output token-identical to spec-off. Warm c1 **1.15x** (82 vs 71.3 tok/s), 20.8% acceptance; upstream gets **1.41x** from the same draft, so half the speedup is unclaimed. Graphed-vLLM A/B owed | `ACTIVE` |
...
| Tenstorrent Blackhole (`BACKEND-TENSTORRENT`) | **NOT APPLICABLE (speed).** Correctness: OPT-125m STRICT 6/6 e2e real HW. [Spec](../.agents/specs/tenstorrent-backend.md) | Device-resident tensors + ttnn::sdpa_decode for real PA perf |
