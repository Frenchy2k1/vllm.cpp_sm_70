...
| Vulkan vs llama.cpp Vulkan (`BENCH-VK-LLAMA`) | **Both arms measured, same weights.** llama.cpp Vulkan `NV_coopmat2`: pp128 **11,514 t/s**, tg32 **160.9 t/s**. vllm.cpp Vulkan: prefill **23.0**, decode **5.85 t/s** | `VK-A2` ON: decode **8.59 -> 59.3 t/s** (**6.9x**), **2.7x** off llama.cpp; 6/6 exact. GPU timestamps: GPU busy only **26%** -- now HOST-bound. VOID: subgroup reduction, Tier-1 fusion, GEMV unroll |
