...
| Tenstorrent Blackhole (`BACKEND-TENSTORRENT`) | **NOT APPLICABLE: no number measured, claimed or owed.** `ACTIVE` W1.2 (6/9 OPT-125m ops, host round-trip per call, no perf design attempted). 8/8 tests PASS real HW. [Spec](../.agents/specs/tenstorrent-backend.md) | Widen op coverage; perf owed only once the round-trip design is revisited |
