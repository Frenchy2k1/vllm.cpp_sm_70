...
- **N5 SPEED PLAN (the user's goal, ≥ vLLM 18.8, both NVFP4):** (1) route the experts to the cutlass sm120a fp4 tensor-core path (the 27B/35B parity kernel — biggest lever), (2) grouped W4A4 MoE (mirror the A3/W9 grouped fold), (3) ResidentNvfp4 device staging, (4) decode CUDA-graph + on-GPU sampling. Repro: `flock $HOME/gpu.lock ./build-cuda/examples/laguna-gen …`; drop_caches first, context-before-load recipe.
