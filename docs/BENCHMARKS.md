...
| Vulkan vs llama.cpp Vulkan (`BENCH-VK-LLAMA`) | **Denominator measured, identical weights.** llama.cpp Vulkan `NV_coopmat2`, Qwen3-0.6B: pp128 **11,514 t/s**, tg32 **160.9 t/s**. **Ours produced NO number** -- never completed at any size tried | **Cause MEASURED**: per-op sync fence-wait (6/6 stacks, ~96 waits/s); compute is ~1-2 s. `VK-A2` is the lever. Open: dispatch COUNT also looks ~10x high |
