...
| Vulkan vs llama.cpp Vulkan (`BENCH-VK-LLAMA`) | **Both arms measured, same weights.** llama.cpp Vulkan `NV_coopmat2`: pp128 **11,514 t/s**, tg32 **160.9 t/s**. vllm.cpp Vulkan: prefill **23.0**, decode **5.85 t/s** | ~27x decode behind (prefill ratio was a METRIC ARTIFACT, fixed). GEMV **1.8x**, argmax **18.9x**, 6/6 exact. VOID: subgroup reduction, Tier-1 fusion. Floor 13%->**49%**; `VK-A2` top lever, needs per-dispatch descriptors |
