...
**Our-engine NVFP4 arm progress (task #230, no throughput owed yet — scaffold):** N1-scaffold LANDED 2026-07-31 — additive `Nvfp4Weight` expert fields on `LagunaMoeWeights` (dead until the N1 loader), CPU build clean + `test_laguna_scaffold` 8/8·167 unchanged. **N1b loader IMPLEMENTED 2026-07-31** (`LoadLagunaForCausalLMWeights`, build-verified, name-map+dtypes verified vs the real `poolside/Laguna-S-2.1-NVFP4` index, `test_laguna_scaffold` 8/8 unchanged) — replaces the throw stub. Run-gates pending: synthetic-fixture CPU test + N2 forward-branch + N4/N5 DGX (correctness + speed-vs-vLLM-W4A4, the real bar which is faster than the 18.8 MARLIN lower bound). No throughput owed yet.
