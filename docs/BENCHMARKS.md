...
| Vulkan vs llama.cpp Vulkan (`BENCH-VK-LLAMA`) | **Both arms measured, same weights.** llama.cpp Vulkan `NV_coopmat2`: pp128 **11,514 t/s**, tg32 **160.9 t/s**. vllm.cpp Vulkan: prefill **23.0**, decode **5.85 t/s** | ~500x prefill / ~27x decode behind. Levers: 71/87 ops on CPU tier, per-op sync dispatch (`VK-A2`), coopmat declines at M=1 |
