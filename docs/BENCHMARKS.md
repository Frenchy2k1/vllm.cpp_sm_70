...
- **N5 SPEED PLAN (the user's goal, ≥ vLLM 18.8, both NVFP4; trace-ranked):** (1) route the experts to the cutlass sm120a fp4 tensor-core path (kills the 99.3% GPU cost — the 27B/35B parity kernel), (2) route the bf16 GEMMs to the GPU via `vt::MatmulBT` instead of the host `MatmulNK` (fastest single win to land first), (3) grouped W4A4 MoE, (4) ResidentNvfp4 staging, (5) decode CUDA-graph + on-GPU sampling. Repro: `flock $HOME/gpu.lock ./build-cuda/examples/laguna-gen …`; drop_caches first, context-before-load recipe.
