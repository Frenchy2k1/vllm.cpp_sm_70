...
| `logprobs_mode` (`SAMPLE-LOGPROB-TOKEN-IDS`, #238) | **NO number measured, claimed or owed.** Correctness-only, CPU. The processed modes add one [n, vocab] device->host copy per step when engaged and nothing when not | No bar is owed: these are observation modes, not a path vLLM optimizes either |
