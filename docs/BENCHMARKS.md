...
| Tenstorrent Blackhole (`BACKEND-TENSTORRENT`) | **NOT APPLICABLE: no number measured, claimed or owed.** `INVENTORIED` proposal, no code, no HW here. [Spec](../.agents/specs/tenstorrent-backend.md) | Row review/acceptance, then a hands-on spike proving the ttnn `Tensor` wrap |
