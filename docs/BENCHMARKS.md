...
| Prompt logprobs (`SAMPLE-PROMPT-LOGPROBS`, #223) | **NO number measured, claimed or owed.** Correctness-only, CPU. Upstream ships this path explicitly unoptimized (`gpu_model_runner.py:5622-5623`); a step where no request asks is unchanged | No bar is owed while vLLM does not optimize it either. If one is wanted, the floor is vLLM's own `prompt_logprobs=k`, same model and prompt |
