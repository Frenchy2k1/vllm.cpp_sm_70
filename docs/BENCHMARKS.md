...
| Tenstorrent Blackhole (`BACKEND-TENSTORRENT`) | **NOT APPLICABLE: no number measured, claimed or owed.** `INVENTORIED`, no `src/vt/` code. Standalone spike matmul'd real HW, bf16 tolerance PASS. [Spec](../.agents/specs/tenstorrent-backend.md) | Row review/acceptance, then real `src/vt/` code |
