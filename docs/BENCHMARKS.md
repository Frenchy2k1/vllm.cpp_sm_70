...
| Pre-Ampere breadth (Turing `sm_75` / Volta `sm_70` / Pascal) | **NO NUMBER OWED OR PRODUCED — nothing executes on these arches.** 2026-08-06 compile audit at `sm_75` (base `249697b7`, nvcc 13.0.88): 20 unconditional CUDA TUs, 18 PASS / 2 FAIL (`cuda_gdn.cu`, `cuda_matmul_nvfp4.cu`). Build-audit evidence only; no library link, no board here, and `sm_70` is not compilable until a CUDA 12.x toolkit is wired | Guard the 2 residual TUs, then port the llama.cpp `fattn-tile`/`fattn-vec` fp16 body. Perf floor when a card exists is **llama.cpp on the same card** (vLLM does not run there) |
