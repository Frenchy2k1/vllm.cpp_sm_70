...
| Vulkan vs llama.cpp Vulkan (`BENCH-VK-LLAMA`) | **Denominator measured, identical weights both sides.** llama.cpp Vulkan `NV_coopmat2`, Qwen3-0.6B F16: pp128 **11,514 t/s**, tg32 **160.9 t/s**. Ours selects Vulkan but is orders slower | **No ratio quoted**: 71 of 87 ops run on the host tier, so it would compare our CPU tier to their Vulkan |
