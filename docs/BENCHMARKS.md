...
| Vulkan vs llama.cpp Vulkan (`BENCH-VK-LLAMA`) | **Denominator measured, identical weights both sides.** llama.cpp Vulkan `NV_coopmat2`, Qwen3-0.6B: pp128 **11,514 t/s**, tg32 **160.9 t/s**. Ours is orders slower and **GPU-bound (96% GPU, 1.6% CPU)** | **No ratio.** Cause UNMEASURED: likely per-op sync dispatch (`VK-A2`, unbuilt), not the CPU tier |
