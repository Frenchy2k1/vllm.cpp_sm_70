...
| Vulkan vs llama.cpp Vulkan (`BENCH-VK-LLAMA`) | **Both arms measured, same weights.** llama.cpp Vulkan `NV_coopmat2`: pp128 **11,514 t/s**, tg32 **160.9 t/s**. vllm.cpp Vulkan: prefill **23.0**, decode **5.85 t/s** | ~500x prefill / ~27x decode behind. Levers MEASURED; two were wrong. CPU tier fires ONCE per run; dispatch is a 0.046 ms floor, so `VK-A2` caps at **1.15x**. GEMM is **55%** of GPU. Argmax **18.9x** (10%->0.6%) |
