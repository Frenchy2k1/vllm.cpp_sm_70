...
| Vulkan vs llama.cpp Vulkan (`BENCH-VK-LLAMA`) | **Denominator measured, identical weights.** llama.cpp Vulkan `NV_coopmat2`, Qwen3-0.6B: pp128 **11,514 t/s**, tg32 **160.9 t/s**. Ours is orders slower | **Cause MEASURED**: per-op synchronous fence-wait (6/6 stacks, ~96 waits/s); compute is ~1-2 s of ~34 min. `VK-A2` is the lever |
