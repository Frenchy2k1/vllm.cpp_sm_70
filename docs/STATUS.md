...
| Qwen3.5-4B plain BF16 direct loading on discrete CUDA | Correctness-complete; throughput passes; latency/VRAM and kernel default gates open | Exact chunks ON: local +2.152% total. Opt-in post-conv tile is byte-exact and 1.859x faster. Opt-in causal-conv K=4 one- and two-channel/thread arms are CUDA byte-exact and share a mutation-gated production/portable dispatcher; production A/B is pending and no speed claim is made |
