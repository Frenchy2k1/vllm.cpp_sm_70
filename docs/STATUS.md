...
| OpenAI server | Supported (subset) | `/v1/completions`, `/v1/chat/completions`, streaming SSE, `/v1/models`, `/health`, `/version`, `/ping`, `/metrics` (Prometheus `vllm:*` names), `/tokenize` (both the raw-`prompt` and the chat-`messages` forms, the latter rendered through the same model chat template as chat-completions), `/detokenize`, `/tokenizer_info` (opt-in, mirrors vLLM's `enable_tokenizer_info_endpoint`; surfaces the tokenizer-config fields the BPE tokenizer can genuinely back), `/server_info`, `/reset_prefix_cache`, `/abort_requests` (abort in-flight requests by id) |
