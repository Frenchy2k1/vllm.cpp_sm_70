...
| Qwen3.5-4B plain BF16 direct loading on discrete CUDA | Correctness-complete; throughput passes; latency/VRAM and kernel default gates open | Exact chunks ON: local +2.152% total. Opt-in post-conv tile is byte-exact and 1.859x faster. Next spike divides the 1.609x causal-conv residual into width specialization and a 256-channel tile, starting with the dominant 279-280-program waves |
