...
| Kimi-Linear-48B-A3B (KDA + NoPE-MLA + MoE hybrid) | W6 DEVICE forward SEAM (CPU-gated) | `ForwardDevice` returns DEVICE-RESIDENT logits (no host download; routing+fusion green); `test_kimi_linear_forward` 7/7·300; residual = DBuf device COMPUTE + e2e golden |
