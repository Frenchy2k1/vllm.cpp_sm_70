...
| Kimi-Linear-48B-A3B (KDA + NoPE-MLA + MoE hybrid) | W1 scaffolding landed (CPU-gated) | W1 registry + config + index-verified name-map + het KV spec + refuse-by-name forward; `test_kimi_linear_scaffold` 9/9. FITS one GB10 (0.77x pool); NET-NEW = KDA kernel + NoPE-MLA + MoE + het-KV forward (W3-W6) |
