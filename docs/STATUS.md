...
| Laguna-S-2.1 MoE (`LagunaForCausalLM`, 118B/8B) | **W1/W2 structural bring-up landed (registered + config parse + name-map + forward scaffold, CPU `-Werror` clean, scaffold-gated); real-model gate PENDING (W4)** | Poolside Laguna: 48 layers (12 global + 36 sliding-window-512), 256 routed top-10 + 1 shared expert, per-head **softplus attention output gate**, sigmoid `noaux_tc` router, dual per-layer RoPE (YaRN full-attn / plain sliding), GQA 8 KV / 128 head-dim, 1M ctx. **W2 (2026-07-30):** `laguna`/`LagunaForCausalLM` registered (`laguna_registry.cpp`), `ParseLagunaParams` resolves the nested dual-rope + variable Q-head + ungrouped sigmoid-noaux MoE config, GGUF `blk.N.*` name-map + UD-Q4_K_XL quant-mix enumerated (Q4_K/Q5_K/Q6_K/Q8_0 ALL already decoded → ZERO new kernel), forward composition scaffolded (`laguna.cpp`, `VT_CHECK(false)` W3 stub); `test_laguna_scaffold` 3/3·40. **W1 oracle DECISION:** vLLM has NATIVE `laguna.py` (in our pin → config constructs); dual-oracle = vLLM-NVFP4/-FP8 behavior (fits GB10 119 GiB; BF16 235 GiB does NOT) + llama.cpp-Q4_K token-exact (same-quant greedy gate). ~85–90% reuse (ds4-MoE + Gemma-sliding + OLMo-3-dual-rope + our Q4_K GGUF keep-quant, ALREADY landed); NEW = per-head softplus attn gate + ungrouped router + name-map + variable-Q-head runner + `poolside_v1` parser. Residuals (W3/W4): device materialization + real forward + strict dual-oracle gate on a fetched checkpoint. See `.agents/specs/laguna-s21-w1w2-2026-07-30.md` (+ W0 `.agents/specs/laguna-s21-scope-2026-07-30.md`) |
