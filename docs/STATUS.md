...
**BINDING 2026-08-04: PARITY with ds4 — 16.28 vs 16.33 tok/s (0.997x, same-session clean). The "~96% / Q8_0 weight-stream floor" below is superseded (nsys-BOTH: per-launch GEMV parity; ds4 is host-launch-bound at 57% busy, ours 97% via the decode graph). HC-expand fusion byte-exact but perf-neutral, held default-OFF; optional f16-DSA beat-path held for gate-class ratification.** **DeepSeek-V4-Flash decode — MHC-pre finish hc=4 register-resident Sinkhorn + norm-fold LANDED: +4.6%, byte-exact, 91.3%→95.5% of ds4 ~16.5 (2026-08-03, `CLAIM-DSV4-MHC-SINK4`, GB10 sm_121a, base `6546227e`).** Delivers `CLAIM-DSV4-MHC-LEAN`'s owed ncu residual. **ncu (locked-clock GB10, M=1) pins the finish as neither compute (SM 0.1%) nor bandwidth (Mem 0.3%) bound but SINGLE-BLOCK serial-latency bound: ~86% of warp-cycles STALL at the `__syncthreads` while thread 0 grinds the 20-iter Sinkhorn through a 2048-byte LOCAL stack frame** (ptxas can't register-allocate the runtime-`hc` `cl[256]/m[256]` arrays; scattered 4-byte-of-32 local-DRAM). This **overturns the "~70 µs irreducible" bound** — the barrier is real but the local-memory Sinkhorn under it was reducible. Fix (byte-exact for the structural hc=4): `MhcPreFinishFloatKernel4` + `SinkhornInplace4` (hardcoded 4×4 + `#pragma unroll` ⇒ **ptxas 2048→0-byte stack frame**, register-resident) + the ds4 norm-fold (`sum += acc*acc` single-pass, no layer_out re-read) — mirroring ds4's `hc4_split_one`/`hc_split_weighted_sum_norm_fused_kernel` (`ds4_cuda.cu:9618/9752`). Gated `VT_V4_MHC_SINK4` (**default ON** per parity-enablers; `=0` = generic finish A/B; only when `hc==4`). **Byte-exact:** standalone `comb`+`layer_out` 0/16 ULP; resident-decode ids **token-IDENTICAL** `=1` vs `=0` (IQ2XXS 85 GiB, "…Paris."). **MEASURED (GB10, worker parked + drop_caches + flock): ncu ~75→~26 µs/call; per-step 62→59 ms (−4.8%); decode 15.07→15.76 tok/s (+4.6%, two OPT reps 15.758/15.747) → 91.3%→95.5% of ds4 ~16.5** (session-stable ~15.87 = ~96%). VERDICT: per-call gap MEASURABLY narrowed (reducible local-memory part gone, byte-exact) — DeepSeek now **~96% of ds4** (session-stable ~15.87 tok/s vs the corrected ~16.5 bar; ~4% BEHIND, not at/above — the Q8_0 proj-GEMV weight-stream floor); the residual (~26 vs ds4 ~7 µs/call) is the fundamental single-block `__syncthreads` floor, closable only by the high-risk layer-chain-spanning MHC restructure the RAW cross-layer dependency blocks (NOT pursued). (Bar history: 17.13 was an unreproduced variance peak; the interim ~16.5 bar and per-lever ratios are preserved in the benchmark record; superseded by the settled 2026-08-04 parity above.)
