...
- **LoRA and multi-GPU** are not supported yet. **Multimodal** is
  ...
  priority; the OpenAI-server wiring has opened — the first CPU brick
  (`CLAIM-MM-SERVING-W1`) parses the OpenAI multimodal content-part array and
  routes the decoded media through the existing processors, but the engine
  mm-feature plumbing and the closing end-to-end GPU gate are still pending, so it
  is not yet servable end-to-end. Image-to-text is strict
