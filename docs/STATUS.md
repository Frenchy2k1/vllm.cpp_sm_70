...
**BINDING 2026-08-04: PARITY with ds4 — 16.28 vs 16.33 tok/s (0.997x), same-session clean single-load both arms; the "~96% behind / Q8_0 weight-stream floor" below is SUPERSEDED (nsys of BOTH engines: our int8 GEMV is at per-launch parity; ds4 does ~1.8x less GPU work but is host-launch-bound at 57% GPU-busy since it graphs only prefill, while ours is 97% busy via the decode CUDA graph). Byte-exact HC-expand fusion held default-OFF (perf-neutral, our graph already collapsed the launches it targets). Optional beat-path: f16 tensor-core DSA/router tensors (near-tie class, held pending gate-class ratification).** Earlier narrative: **DeepSeek-V4-Flash decode — MHC-pre finish hc=4 register-resident Sinkhorn + norm-fold LANDED: +4.6%, byte-exact, 91.3%→95.5% of ds4 ~16.5 (2026-08-03, `CLAIM-DSV4-MHC-SINK4`, GB10 sm_121a, base `6546227e`).** Delivers `CLAIM-DSV4-MHC-LEAN`'s owed ncu residual. **ncu (locked-clock GB10, M=1) pins the finish as neither compute (SM 0.1%) nor bandwidth (Mem 0.3%) bound but SINGLE-BLOCK serial-latency bound: ~86% of warp-cycles STALL at the `__syncthreads` while thread 0 grinds the 20-iter Sinkhorn through a 2048-byte LOCAL stack frame** (ptxas can't register-allocate the runtime-`hc` `cl[256]/m[256]` arrays; scattered 4-byte-of-32 local-DRAM). This **overturns the "~70 µs irreducible" bound** — the barrier is real but the local-memory Sinkhorn under it was reducible. Fix (byte-exact for the structural hc=4): `MhcPreFinishFloatKernel4` + `SinkhornInplace4` (hardcoded 4×4 + `#pragma unroll` ⇒ **ptxas 2048→0-byte stack frame**, register-resident) + the ds4 norm-fold (`sum += acc*acc` single-pass, no layer_out re-read) — mirroring ds4's `hc4_split_one`/`hc_split_weighted_sum_norm_fused_kernel` (`ds4_cuda.cu:9618/9752`). Gated `VT_V4_MHC_SINK4` (**default ON** per parity-enablers; `=0` = generic finish A/B; only when `hc==4`). **Byte-exact:** standalone `comb`+`layer_out` 0/16 ULP; resident-decode ids **token-IDENTICAL** `=1` vs `=0` (IQ2XXS 85 GiB, "…Paris."). **MEASURED (GB10, worker parked + drop_caches + flock): ncu ~75→~26 µs/call; per-step 62→59 ms (−4.8%); decode 15.07→15.76 tok/s (+4.6%, two OPT reps 15.758/15.747) → 91.3%→95.5% of ds4 ~16.5** (session-stable ~15.87 = ~96%). VERDICT: per-call gap MEASURABLY narrowed (reducible local-memory part gone, byte-exact) — DeepSeek now **~96% of ds4** (session-stable ~15.87 tok/s vs the corrected ~16.5 bar; ~4% BEHIND, not at/above — the Q8_0 proj-GEMV weight-stream floor); the residual (~26 vs ds4 ~7 µs/call) is the fundamental single-block `__syncthreads` floor, closable only by the high-risk layer-chain-spanning MHC restructure the RAW cross-layer dependency blocks (NOT pursued). **BAR CORRECTION (2026-08-04): the ds4 denominator is ~16.5 tok/s, NOT 17.13.** All `% of ds4` figures above are now against the fair same-session ds4 bar **~16.5 tok/s** (`ds4-bench.csv gen_steady_tps 16.5`, flat across ctx 512-2048 / n 60-200 / the GB10-no-op reserve config). The prior 17.13 anchor was an UNREPRODUCED variance-peak (box max 16.72, no measurement log, introduced as a bare denominator in `d5dbd581` with no fresh ds4 run — ds4's variance-peak / GPU-active-17.6 figure). ds4's own greedy swings ~9% run-to-run (15.39 low outlier ↔ 16.72); the 15.39 is ds4's WORST session, NOT our bar. Ours session-stable ~15.87 tok/s = ~96%, ~4% behind. Per-lever ratios recomputed onto ~16.5 (measured tok/s unchanged). See docs/BENCHMARKS.md `CLAIM-DSV4-MHC-SINK4`.
