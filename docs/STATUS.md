...
| Kimi-Linear-48B-A3B (KDA + NoPE-MLA + MoE hybrid) | W2-W6 CPU reference forward landed (CPU-gated) | CPU ref `Forward` composes the 27-layer hybrid (KDA/kimi_kda, NoPE-MLA, sigmoid-noaux MoE, dense); `test_kimi_linear_forward` 6/6·246; DEVICE forward refuses; residual = DEVICE forward + e2e golden |
