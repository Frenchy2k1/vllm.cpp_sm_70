...
| CPU backend vs llama.cpp | 20-core Arm at floor; RPi5/A76 below floor `GATING`; **x86_64 RSS at floor, speed `PENDING`** | Pi: AAPCS64 beats SDOT 3.66-5.08%; llama.cpp 2.17x pf / 1.53x dec faster (0.461x/0.653x); RSS -24.2%; 64-tok byte-exact. BF16 GEMM open. x86_64 first measured 2026-08-11 (#433): peak RSS 1.0022x PARITY, throughput pending a quiet host, quant path portable-tier only (CIQ `G5`) |
