...
| OpenAI server | Supported (subset) | `/v1/completions`, `/v1/chat/completions`, streaming SSE, `/v1/models`, `/health`, `/version`, `/ping`, `/metrics` (Prometheus `vllm:*` names), `/tokenize` (both the raw-`prompt` and the chat-`messages` forms, the latter rendered through the same model chat template as chat-completions), `/detokenize`, `/server_info`, `/reset_prefix_cache` |
