...
**BINDING 2026-08-05: `VT_V4_RESIDENT_W` (default-OFF, recommend-flip) BEATS ds4 — 18.69 vs 16.33 tok/s (1.144x), byte-exact: the dense Q8_0 MLA/shared/lm_head proj tower was read from GGUF-mmap over ATS; staging it `cudaMalloc`-device once (nsys A/B, ids-IDENTICAL: Q8_0 per-launch 184→147, GroupDiag 212→166, Pair 74→60 µs, ~20% each) lifts decode 16.23→18.69 (median-of-3, drop_caches, PEAK flat 86.68 GiB — mmap pages evict, net move not add). Mirrors Laguna `VT_LAGUNA_RESIDENT_BF16W`. OVERTURNS the "per-launch GEMV parity / Q8_0 weight-stream floor" framing below — our GEMV was ATS-bound, not at ds4 parity; residency is the transfer lever. Default arm stays 16.28 (0.997x) until flip.** Prior bindings (superseded, full detail in the benchmark record): PARITY 16.28 vs 16.33 (0.997x, nsys-BOTH per-launch GEMV, decode-graph 97% busy vs ds4 host-bound 57%); MHC-pre hc=4 register-resident Sinkhorn `VT_V4_MHC_SINK4` default-ON +4.6% byte-exact (`CLAIM-DSV4-MHC-SINK4`, ncu ~75→~26 µs finish); HC-expand fusion + optional f16-DSA beat-paths held default-OFF.
