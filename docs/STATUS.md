...
| MiniMax-H3 (`MiniMaxH3DiTModel`, omni-modal video+audio DIFFUSION) | **Portable lane COMPLETE; e2e video RAN (Thor); FP4 speed pending sm_121** | Portable path COMPLETE, CPU/Thor-gated (DiT 1.6e-7, both VAEs, encoder, t2va, GGUF+NVFP4 loaders, /v1/videos); e2e video on real weights (Thor); speed = NVFP4 FP4 path, sm_121-gated. Detail: model-matrix + ledger. |
