...
| GGUF loading (F32/F16/BF16/Q4_0/Q8_0/Q2_K/Q3_K/Q4_K/Q5_K/Q6_K/IQ2_XXS/NVFP4) | Supported; compute-in-quant on CPU for the six K-block encodings; Q2_K/IQ2_XXS/NVFP4 are materialize-only (dequant to bf16) | Weights in six block encodings stay compressed from file to matmul on CPU (no BF16 expansion). NVFP4 (ggml type 40) now DEQUANTIZES, including the per-tensor (per-expert) `<stem>.scale` sidecar the container keeps outside the blocks; gated BIT-EXACT against the compressed-tensors NVFP4 path on real Qwen3.6-27B bytes from both containers. It expands to bf16 (no NVFP4 GGUF GEMM). An NVFP4 GGUF model HAS now been run end to end (Qwen3.6-27B, dgx GB10, coherent greedy output, and as a DFlash speculation target); because the expansion computes in bf16 it is a numerically different target from the safetensors sibling of the same quantization run, diverging at token 4 on a greedy prompt. **Q2_K (id 10) + IQ2_XXS (id 16) now DEQUANTIZE (2026-07-29, `CLAIM-DSV4-GGUF-LOADER`):** the ~2-bit types the single-Spark `DeepSeek-V4-Flash-GGUF UD-IQ2_XXS`/`UD-Q2_K_XL` vehicles use, ported 1:1 from llama.cpp `ggml-quants.c` (`iq2xxs_grid` codebook + signs; Q2_K nibble sub-scale/min), unit-gated on hand-derived bytes (`test_gguf_dequant` 15/15). Dequant-only (no vec_dot → expand-bf16). A V4-GGUF model still cannot RUN: the V4-GGUF name map (tensor-manifest-blocked) + the V4 forward (W3-W8) remain |
...
| GGUF loading (F32/F16/BF16/Q4_0/Q8_0/Q2_K/Q3_K/Q4_K/Q5_K/Q6_K/IQ2_XXS/NVFP4) | Supported; compute-in-quant on CPU for the six K-block encodings; Q2_K/IQ2_XXS/NVFP4 are materialize-only (dequant to bf16) | Weights in six block encodings stay compressed from file to matmul on CPU (no BF16 expansion). NVFP4 (ggml type 40) now DEQUANTIZES, including the per-tensor (per-expert) `<stem>.scale` sidecar the container keeps outside the blocks; gated BIT-EXACT against the compressed-tensors NVFP4 path on real Qwen3.6-27B bytes from both containers. It expands to bf16 (no NVFP4 GGUF GEMM), and no NVFP4 GGUF model has been run end to end. **Q2_K (id 10) + IQ2_XXS (id 16) dequant landed 2026-07-29** (`CLAIM-DSV4-GGUF-LOADER`, the single-Spark DeepSeek-V4-Flash GGUF vehicles; dequant-only, `test_gguf_dequant` 15/15); a V4-GGUF RUN still needs the V4-GGUF name map + forward (W3-W8) |
...
| Pooling task class (embeddings / classify / score / rerank) | Spiked; first CPU brick landed (pooler op), not yet servable end-to-end | The non-generative task class. W0 spike over the whole vLLM pooling surface (Pooler abstraction, pooling runner, task taxonomy, `/v1/embeddings` + score/rerank/classify endpoints, concrete models) is committed (`.agents/specs/pooling-task-class.md`, `CLAIM-POOLING`). W1 landed the pooler OP on the CPU backend: the sequence pooling methods CLS/LAST/MEAN (+ factory) over a packed `[num_tokens, hidden]` buffer keyed by a `PoolingCursor` (CLS/MEAN reject partial prefill, LAST allows it, MeanPool upcasts to float32) and the activation heads Identity / Normalize (L2) / MultiLabelClassify (sigmoid) / Classify (sigmoid or softmax) — unit-gated vs double-precision references, RED-first. NOT yet servable: the pooler heads composite + DispatchPooler (W2), the pooling runner + a concrete pooling model (W3), the endpoints (W4), and tokwise AllPool/StepPool (W5) are named residuals. See docs/BENCHMARKS.md |
...
Qwen3/dense k-quant both engines already load. **Our-side quant-type blocker CLEARED
(2026-07-29, `CLAIM-DSV4-GGUF-LOADER`):** IQ2_XXS (id 16) + Q2_K (id 10) now dequant
(ported 1:1 from llama.cpp `ggml-quants.c`, `test_gguf_dequant` 15/15). The GGUF header
was HTTP-range-verified (`general.architecture=deepseek4`, `general.file_type=19`=IQ2_XXS,
`split.tensors.count=1328`, full `deepseek4.*` config-KV schema). The V4 registry GGUF
reject STAYS: the V4-GGUF `blk.N.*` name map needs the tensor manifest (beyond the CDN
range cap + uncached, 90 GB download prohibited), and the V4 forward (W3-W8) is unbuilt. **W3 attention primitives landed
