...
| `VLLM_CPP_TRITON` | computed: `ON` for a CUDA build with the vendored trees present, else `OFF` | Consume the vendored per-arch Triton-AOT GDN cubins (CUDA only; no Python needed). It ships ON because the artifacts are pre-generated cubins embedded in plain C — a C compiler is the whole requirement. It declines, with one `STATUS` line naming the condition, when `VLLM_CPP_CUDA` is `OFF`, when `VLLM_CPP_TRITON_REGEN` is `ON`, or when a vendored tree is absent. Turn it off with `-DVLLM_CPP_TRITON=OFF` |
