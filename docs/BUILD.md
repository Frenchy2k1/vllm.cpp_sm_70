...
# Volta (Tesla V100, sm_70): NVFP4 GEMM + FA2 decode fast paths, needs nvcc < 13
cmake -S . -B build-cuda-sm70 -DVLLM_CPP_CUDA=ON \
  -DVLLM_CPP_CUDA_ARCHITECTURES=70
```

Of these, only `sm_87` (Orin), `sm_110` (Thor) and `sm_70` (Volta, V100) have
been run on real hardware. `sm_80/86/89`, `sm_90a` and `sm_100a/103a` are
build-verified: they compile `-Werror`-clean and emit the expected SASS, but no
board here has executed them. `sm_70` (Volta) is runtime-proven on 4x V100:
the sm70 NVFP4 W4A16 decode GEMM and the sm70 FA2 decode fast paths are the
branch's shipped kernels (fp16 `mma.sync`, no bf16/fp8), with the
`sm70-nvfp4-gemm` / `sm70-fa2-v1` feature-table rows and token parity against
the Milestone-0 oracle. Build requires nvcc < 13 (CUDA 12.9.x is the last).
...
from the other boards are welcome.
