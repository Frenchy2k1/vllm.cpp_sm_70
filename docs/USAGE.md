...
`fp4-mma` stay off there because no kernel body exists for it. `70` (Tesla
V100, Volta) builds the NVFP4 W4A16 decode GEMM (`sm70-nvfp4-gemm`) and the
FA2 fragment-core + paged decode (`sm70-fa2-v1`); it needs a CUDA `< 13`
toolkit (nvcc 13 dropped `sm_70` codegen). `cmake -P
