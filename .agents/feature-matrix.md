...
| `MODEL-MM` | 115 v0.25.0-target multimodal IDs | `models/registry.py:331-583` | `PARTIAL` | image+video e2e (Qwen3-VL-4B + 27B `Qwen3_5ForConditionalGeneration` STRICT 32/32) + audio e2e (Voxtral-Mini-3B 14/14) + Whisper encoder 203/203 LANDED; processors + encoder-cache present; speed: tower lever #1 + decode lever #2 (on-GPU greedy argmax + no embed round-trip, bit-exact, 2026-07-27 `CLAIM-MULTIMODAL-SPEED-DECODE`) CLOSED; 35B ViT, batched/graphed mm serving (lever #3) and broader mm breadth pending | model matrix |
