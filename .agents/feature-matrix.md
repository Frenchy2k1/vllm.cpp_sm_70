...
| `/tokenize`, `/detokenize`, `/ready`, `/ping`, `/server_info`, `/reset_prefix_cache` | various routers | `ACTIVE` T1 | **LANDED + CPU-GATED 2026-07-27 (`SERVE-UTILITY-ENDPOINTS`, `CLAIM-ROADMAP-C8`):** `/tokenize` (prompt form) + `/detokenize` + `/ping` + `/server_info` + `/reset_prefix_cache` (injected callback), all additive/opt-in on the ApiServer, schema-matched to vLLM 0.26. **CHAT-FORM `/tokenize` LANDED + CPU-GATED 2026-07-28 (`CLAIM-C8-CHAT-TOKENIZE`):** `/tokenize` accepts BOTH arms of the `TokenizeRequest` union — chat form (`messages`+flags) renders through the SAME chat template `create_chat_completion` uses (`chat_.prompt_fn()`) then tokenizes, exact-gated tokens == render→Encode, RED-first (`test_api_server` 27/27, 337). Residual: `chat_template_kwargs`/`continue_final_message` full render passthrough, `/tokenizer_info`, `/ready`, full server_info config dump | [specs/utility-endpoints.md](specs/utility-endpoints.md) |
