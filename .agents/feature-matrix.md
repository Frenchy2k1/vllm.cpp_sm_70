...
| Prefix caching (APC) | `v1/core/kv_cache_utils.py`, `v1/core/kv_cache_coordinator.py` | `PARTIAL` T0 | W0 ports no-prefix coordination, hybrid/attention-free default-off resolution, explicit CLI override, and cache-off benchmark contracts; hashing/lookup/reuse + hit-rate stats (W1) exist; **W2 DONE 2026-07-27**: block-hash `extra_keys` (mm hash / LoRA name / `cache_salt`) ported 1:1 + no-false-share RED-proven (unblocks MM/LoRA cache consumers). Partial-block/retention/connector breadth + the cache-ON model gate (W3, RI) remain | [prefix-prompt-caching-parity.md](specs/prefix-prompt-caching-parity.md) |
