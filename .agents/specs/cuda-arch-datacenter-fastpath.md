...
|---|---|---|---|---|
| DC1 ✅ **LANDED (DERIVED+BUILD-VERIFIED 2026-07-28, `CLAIM-CUDA-SM100-NVFP4`)** | **NVFP4 scaled-mm sm100 (tcgen05)** — a faithful 1:1 TYPE-port of vLLM's `Fp4GemmSm100` (`ArchTag=Sm100` + `KernelScheduleAuto`/`EpilogueScheduleAuto` → CUTLASS 4.5.0 selects the tcgen05 block-scaled collective; `sm100_fp4_config_{default,M256,M16}` tile/cluster/PerSm shapes) at [`cuda_matmul_nvfp4_sm100.cu`](../../src/vt/cuda/cuda_matmul_nvfp4_sm100.cu), gated by a NEW dedicated `cutlass-nvfp4-sm100` FEATURE-TABLE cell (100a-only). NOTE: NOT a plain `Sm120→Sm100` swap of our sm_12x body — that body is `ArchTag=Sm120`+`KernelTmaWarpSpecializedCooperative` (FlashInfer sm120 template, a DIFFERENT collective) and the native `fp4-mma` `mma.sync kind::mxf4nvf4` is consumer-Blackwell-only; the datacenter leg needs vLLM's `KernelScheduleAuto` tcgen05 structure + its own cell so the sm120 sweep never compiles for compute_100a. | `SM100`, `FP4` (sm100 leg) | **S** | **BUILD-VERIFY (dgx nvcc 13.0 + cutlass 4.5.0, COMPILE-ONLY):** single-arch `-arch=sm_100a` `-DVT_CUTLASS_NVFP4_SM100=1 --expt-relaxed-constexpr --expt-extended-lambda -diag-suppress=20012 -isystem cutlass` → 0-warn EXIT=0; `cuobjdump -lelf` real `cuda_matmul_nvfp4_sm100.1.sm_100a.cubin`. RED (HEAD 100a→`cutlass-nvfp4 DISABLED`), sm_121a neutral (cell DISABLED + TU inert). No B200 board ran it |
| DC2 🟢 **sm90 + sm100 FP8 LEGS LANDED (DERIVED+BUILD-VERIFIED 2026-07-28, `CLAIM-CUDA-SM90-C3X` + `CLAIM-CUDA-SM100-C3X` [DC3])** | **CUTLASS scaled-mm C3x FP8/INT8 + blockwise, sm90 AND sm100.** The Hopper **FP8** leg is a faithful 1:1 type-port of vLLM's `cutlass_3x_gemm_sm90_fp8` (`ArchTag=Sm90` + `KernelTmaWarpSpecialized{Pingpong,Cooperative,}FP8FastAccum` + `EpilogueSchedule=TmaWarpSpecialized{,Cooperative}` → CUTLASS 4.5.0 emits the 4th-gen wgmma/TMA warp-specialized collective; the exact `sm90_fp8_config_{default,M8192_K6144,M64_N8192}` tile/cluster/schedule shapes) at [`cuda_scaled_mm_c3x_sm90.cu`](../../src/vt/cuda/cuda_scaled_mm_c3x_sm90.cu), gated by a NEW dedicated `scaledmm-c3x-sm90` FEATURE-TABLE cell (90a-only, NOT a widening of the sm_12x `cutlass-fp8` cell — the sm120 scaled-mm PTX ptxas-rejects for compute_90a). The **datacenter-Blackwell sm100 FP8** leg (the sequential "DC3" brick, `CLAIM-CUDA-SM100-C3X`) is a faithful 1:1 type-port of vLLM's `cutlass_3x_gemm_sm100_fp8` (`ArchTag=Sm100` + `KernelScheduleAuto`/`EpilogueScheduleAuto` → CUTLASS 4.5.0 selects the 5th-gen **tcgen05** collective, NOT an explicit FP8FastAccum schedule; `sm100_fp8_config_{default,M256,M64}` tile/cluster shapes — the datacenter default is a 2SM `ClusterShape<_2,_2,_1>`, distinct from Hopper's `<_2,_1,_1>`) at [`cuda_scaled_mm_c3x_sm100.cu`](../../src/vt/cuda/cuda_scaled_mm_c3x_sm100.cu), gated by its OWN dedicated `scaledmm-c3x-sm100` FEATURE-TABLE cell (100a-only, NOT a widening of `cutlass-fp8` NOR `scaledmm-c3x-sm90`). RESIDUALS: the sm90/sm100 **int8** C3x legs (`scaled_mm_sm{90,100}_int8.cu`, non-FP8FastAccum) + the sm90/sm100 **blockwise-fp8** legs are separate later bricks. | `SM090`, `SM100`, `SCALEDMM-C3X` | S–M | **BUILD-VERIFY (dgx nvcc 13.0 + cutlass 4.5.0, COMPILE-ONLY):** single-arch `-arch=sm_90a` `-DVT_SCALEDMM_C3X_SM90=1 --expt-relaxed-constexpr --expt-extended-lambda -diag-suppress=20012 -isystem cutlass` → 0 compiler warnings EXIT=0 (`ptxas info C7510` names `wgmma.mma_async` + the `Sm90` warp-specialized FP8FastAccum kernels); `cuobjdump -lelf` real `cuda_scaled_mm_c3x_sm90.1.sm_90a.cubin`. Configure `90a`→`scaledmm-c3x-sm90 ENABLED`. RED (HEAD table had no cell — `vt_cuda_feature_archs` fatal `unknown CUDA feature`). sm_121a neutral (cell DISABLED + `cutlass-fp8` still ENABLED for 121a + TU inert). No Hopper board ran it. **sm100 FP8 leg (DC3, base `65b0d522`):** single-arch `-arch=sm_100a` `-DVT_SCALEDMM_C3X_SM100=1 …-diag-suppress=20012 -isystem cutlass -Werror all-warnings` → 0-warn EXIT=0; `cuobjdump -lelf` real `cuda_scaled_mm_c3x_sm100.1.sm_100a.cubin` (+ bonus tcgen05 proof: 3 `Sm100TmaUmmaWarpSpecialized` kernels naming `SM100_MMA_F8F6F4_{SS,2x1SM_SS}`, `SM100_TMA_2SM_LOAD_MULTICAST`, `SM100_TMEM_LOAD`; SASS `LDTM`/`tmem`). Configure `100a`→`scaledmm-c3x-sm100 ENABLED`. RED (HEAD 100a→fatal `unknown CUDA feature 'scaledmm-c3x-sm100'`). sm_121a neutral (cell DISABLED + `cutlass-fp8`/`cutlass-nvfp4`/`cutlass-nvfp4-sm100` byte-unchanged + TU inert, 0 tcgen05 symbols vs 3). No B200 board ran it |
