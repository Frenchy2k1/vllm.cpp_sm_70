...
7. **`ROAD-V1-C8` /metrics + streaming parser + utility endpoints** `[H]` (oldest T0
   ...
   +1/15 + `test_llm_engine` +1/26, RED-first). Remaining W:
   JSON-schema arg coercion; the chat/completion RESPONSE-BODY timing surface;
   AsyncLLM serving-path metric wiring + config-gated families.
   **Gate:** metric-name
