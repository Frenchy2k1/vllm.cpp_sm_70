...
| `ROAD-V1-C5` sliding/YaRN | **RI** | Joint spike accepted; all W1–W8 leaves implemented and **CPU/oracle/sanitizer green** (`ATTN-YARN`, `ATTN-ROPE-{LLAMA3,LONGROPE,DYNAMIC-NTK}`, `ATTN-SLIDING-WINDOW`, `ATTN-CHUNKED-LOCAL`, `KV-{SLIDING-WINDOW,CHUNKED-LOCAL}-SPEC` — all GATING). | Compile/run the shared scaled-RoPE + local-mask CUDA path on GB10 and pass the feature-positive **model/oracle/trace/every-axis** gates (all still open). Unblocks long-context + sliding-window model families. |
...
| `ROAD-V1-C7` sampling/logprobs | **RI** | `SAMPLE-CORE`, `SAMPLE-LOGPROBS`, `SAMPLE-LOGIT-FILTERS` (logit_bias/allowed_token_ids/bad_words) all implemented (PARTIAL). | Full param-surface + end-to-end payload parity gates; `prompt_logprobs`, logprobs_mode variants, philox-RNG, beam-search, custom logits processors (INVENTORIED). Headline API-parity item. |
...
2. **`ROAD-V1-D4-APC` W3 cache-ON gate + W2 extra_keys** `[H]` ("same featureset and
   better"; W2 unblocks MM + LoRA cache consumers). W: run the first cache-ON dense
   gate; then `generate_block_hash_extra_keys` (mm/LoRA/salt), `cache_salt`,
   `skip_reading_prefix_cache`. **Gate:** token-exact ON/OFF/oracle + every-axis
   cache-on grid vs vLLM. **Size S→M.** **Vehicle:** a dense model on DGX.
