...
| Offline Batch API (`/v1/batches`, file runner) | `vllm/entrypoints/openai/run_batch.py:793` (`run_batch`), input/output `:148,:213`; batched chat/embed/score/transcription/translation | RECORDS-GAP — no row | M | Async bulk job format; recommend `SERVE-BATCH-API`. |
