...
### D14 RESULT (2026-07-27, `CLAIM-DFLASH-D14`) — **SPEED GATE MET → SPEC-DFLASH `DONE`. The D13 ~2% residual was the from-scratch `DFlashPagedBlockAttention` draft kernel; ported to a WARP-scoped online-softmax variant (3.1x faster) → our-ON graphed ≥ vLLM-ON graphed (non-overlapping 3-rep bands, 1.003x). Correctness + inertness UNCHANGED. Block-diffusion drafting is now correctness-complete (ratified near-tie) AND at/above vLLM throughput — the roadmap's FINAL open speed item is closed.**

Base `origin/main` `4657f6f3` (branch `dflash-d14`); dgx GB10 sm_121a, pin `555967922`/vLLM 0.26.0.dev0, `~/venvs/vllm-oracle`; reused build tree `~/work/dflash-d12/tree/build-cuda`; all GPU under one `flock $HOME/gpu.lock`. NOT pushed. Diff is a single source file (`src/vt/cuda/cuda_ops.cu`, additive: one new `__global__` + one env-gate helper + a launcher branch; the D12/D13 block kernel is retained under `VT_DFLASH_ATTN_BLOCK=1`).

**W0 — nsys attribution (measure, don't guess).** `nsys profile -t cuda --cuda-graph-trace=node` of OUR graphed spec-on step + `cuda_gpu_kern_sum` + a per-call sqlite histogram, cross-referenced with a vLLM-ON torch-profiler run (`ProfilerConfig(profiler="torch")`) and an OUR spec-OFF profile (to isolate draft-only kernels). Findings:
- The 27B target (a Qwen3-Next-style **hybrid GDN + MoE + full-attn** NVFP4 model) dominates GPU time via `nvjet_sm121` fp8 + `cutlass_fp4_blockscaled_Sm120` + `GdnSpecDecodeFusedKernel` — all TARGET, at SACRED throughput parity, NOT the gap.
- **The draft bf16 GEMMs (M=(1+k)=17) run `cutlass_80_wmma_tensorop_bf16` in BOTH our engine AND vLLM** (vLLM's `F.linear`→cublasLt selects the same sm_80 kernel). So the GEMMs are NOT a divergence — the "sm_80 vs nvjet" hypothesis was REFUTED by profiling vLLM.
- **The divergence is the draft ATTENTION.** OUR from-scratch `DFlashPagedBlockAttentionKernel` = **242.9 ms total (n=430) = 1.8% of the 13.7 s GPU total, median ~460 us/call**. vLLM's fused flash draft-attn is **~0.15%**. This single-kernel gap accounts for essentially the entire D13 ~2% residual (spec-OFF confirmed the kernel is draft-only). Root cause: grid `(nq=17, hq=32)`, **kBlock=256 threads per (query,head) block looping SERIALLY over C~500-640 keys** with a 256-wide shared-mem tree reduction + 2 `__syncthreads` PER KEY — the latency-bound sync/memory storm the ViT tower fixed with `AttentionDenseFast`.

**W1 — the close (1:1 with our own proven warp-attn structure).** `DFlashPagedBlockAttentionWarpKernel<Tin,Tout>`: ONE WARP per (block-query row, q-head), head_dim dot product via `__shfl_xor_sync` butterfly (no `__syncthreads`), register accumulator (`kMaxPerLane=8`, head_dim≤256), per-lane online-softmax stats — the SAME structure as the shipped `AttentionWarpKernel` (grounded in `src/vt/cuda/flash_attn/`), with the IDENTICAL paged/block combined-index read, `btbl`/`seq_lens` indirection, causal/SWA `jlo..jhi` mask, and GQA broadcast copied verbatim from the block kernel. Default ON; `VT_DFLASH_ATTN_BLOCK=1` selects the bit-identical block kernel. NOT bit-identical to the block kernel, but the SAME f32-online-softmax math within the CUDA==CPU envelope, and **spec-decode output is exact by construction** (the target verify is untouched; only which draft proposals are accepted can shift, within the ratified ±4 gate).

**RESULT.** Draft attention **242.9 → 77.9 ms (3.1x, −68%)**; our-ON c1 throughput **28.60 → 29.32 tok/s (+2.5%)**.
- **THE SPEED GATE (MET).** FINAL same-session 3-rep A/B (8 prose+code prompts × 256 tok, input 512, c1, greedy, cold leg discarded, one `flock`): **our-ON graphed 29.42 / 29.27 / 29.32 tok/s (median 29.32)** vs **vLLM-ON graphed MRV2 29.240 / 29.247 / 29.233 (median 29.240)**. Our WORST rep (29.27) exceeds vLLM's BEST (29.247) — non-overlapping bands — **1.003x ⇒ our-DFlash-ON ≥ vLLM-DFlash-ON. GATE MET.** (Our OFF 10.16 ≥ vLLM OFF.)
- **Correctness (UNCHANGED).** e2e `test_qwen27_dflash_spec_decode` **27/27** with the graph (VT_DFLASH_GRAPH=1) AND eager (=0) — BIT-IDENTICAL to each other and to D13: same divergences (France@11 `got[4252,13,2972,57590,332]`, 17×23@12 `got[18,53031,567,628,1387]`), 2/4 STRICT, **acceptance 19/39/29/25 unchanged** (draft accepted **1629 identical** warp-vs-block across the entire 8×256 A/B set — the ULP change flipped ZERO acceptances). `test_ops_dflash_paged_block_attn` CUDA==CPU **795648/795648** + **compute-sanitizer memcheck 0 errors**.
- **Inertness.** DFlash-off: 27B SACRED `test_qwen27_paged_engine` **235/235** + 27B MTP `test_qwen27_spec_decode` **9/9** byte-identical (the warp kernel is DFlash-only). CUDA `-Werror` clean; `check-device-leakage` not increased. `benchmark_binding=true`.
- **VERDICT: SPEC-DFLASH → `DONE`.** No irreducible ceiling — the residual was a concrete, portable single-kernel lever, closed. Raw nsys: `/tmp/d14_ours_specon.nsys-rep` (block) + `/tmp/d14_ours_warp.nsys-rep` (warp); timing harness `vllm_dflash_timing3.py`.

### D13 RESULT (2026-07-27, `CLAIM-DFLASH-D13`) — **Part C LANDED + GPU-GATED: fixed-capacity PAGED draft-KV store wired through `vt::DFlashPagedBlockAttention` + the static-shape draft-step CUDA GRAPH (capture + replay). Capture-correctness PROVEN (replayed == eager BIT-IDENTICAL). c1 speed A/B: our-DFlash-ON 28.70 tok/s vs vLLM-DFlash-ON 29.35 tok/s (tight 3-rep band) = 0.978x — NEAR-PARITY, ours ~2% BELOW; the gap CLOSED 0.917x (D9) -> 0.978x (D13) via the paged read (the CUDA graph is perf-neutral). The ≥vLLM speed bar is NOT met (a ~2% residual remains); SPEC-DFLASH stays `ACTIVE`.**

D13 executed the D11/D12 Part-C turnkey. Base `origin/main` `c7ca1a4a`; isolated worktree; reused dgx tree `~/work/dflash-d12/tree/build-cuda` (GB10 sm_121a, pin `555967922`/vLLM 0.26.0.dev0, `~/venvs/vllm-oracle`, all GPU under `flock $HOME/gpu.lock`). NOT pushed. Diff is a single file (`src/vllm/model_executor/models/qwen3_dflash.cpp`, +368/-58), additive + config-gated.

**Part C.1 — fixed-capacity PAGED draft-KV store (LANDED, bit-identical).** `DflashDeviceKVStore` is now a fixed-capacity paged cache: per draft layer a persistent pool `[max_pages, block_size=16, Hkv, Dh]` bf16, plus a persistent identity `block_table [1,max_pages]` and `seq_lens [1]`. A context row at absolute position p lives at flat slot p, so `AppendContextKVDevice` is a plain `vt::IndexCopy` scatter into slots `[num_ctx, num_ctx+count)` — BIT-IDENTICAL bf16 bits to the D9/D11 contiguous store (same projection, same positions, just placed at fixed paged slots). Shapes are STATIC; the growing context enters purely as the in-place `seq_lens` value + block_table DATA.

**Part C.2 — capture-safe static-shape forward through the Part B paged kernel (LANDED).** `ForwardPagedBody` runs the single-request (1+k) block through `vt::DFlashPagedBlockAttention` reading the persistent paged store — NO materialized `[context;block]` concat, NO function-local host index-map uploads (the [[cudagraph-capture-bakes-stack-addresses]] class), block-query outputs emitted directly (no IndexSelect). The runner's P==1 propose path (the c1 gate + the e2e, which drives one request at a time) routes through it; P>1 keeps a bit-identical materialized fallback. Default ON (`VT_DFLASH_PAGED`).

**Part C.3 — the draft-step CUDA GRAPH (capture + replay, LANDED).** Per request, the first propose step WARMS the shared DevicePool with one eager `ForwardPagedBody` (returns that step's output) then `BeginCapture -> ForwardPagedBody -> EndCaptureGraph` over persistent buffers (`g_hidden` embed target refreshed OUTSIDE the graph, `g_dpos` refreshed in place, `g_cu` constant, the paged store + `seq_lens`/`block_table` persistent); every later step `ReplayGraph`s, the growing context entering purely via the in-place `seq_lens`. The warm-in-step pass is REQUIRED: a full 27B target verify forward + KV append runs between draft steps and perturbs the shared pool, so a prior-step warm does NOT guarantee the free-list holds the draft's size classes at capture time (a `Get` miss -> `cudaMalloc` is forbidden mid-capture — the first observed failure, fixed by warming immediately before `BeginCapture`). Config-gated `VT_DFLASH_GRAPH` (default ON).

**Capture-correctness (MANDATORY GATE — MET).** `test_qwen27_dflash_spec_decode` 27/27 with the graph path (VT_DFLASH_GRAPH=1) producing tokens BIT-IDENTICAL to the eager path (VT_DFLASH_GRAPH=0) — a direct same-run diff: BOTH arms give France diverge@11 `got[4252,13,2972,57590,332]`, 17x23 diverge@12 `got[18,53031,567,628,1387]`, fibonacci+three-laws STRICT, acceptance **19/39/29/25** — identical to D5/D7/D9/D12. The graph demonstrably ENGAGED (5 captures at C=2048/5/4/15/6, 32+ replays counted), and replays across DIFFERENT growing C confirm the context-as-data paged design. Per [[cudagraph-capture-bakes-stack-addresses]] the token diff (not a clean sanitizer run) is the proof, and it holds.

**THE c1 SPEED A/B (foreground-measured, one `flock` series, cold rep discarded, 8 prose+code prompts x 256 tok, input 512, greedy):**

| axis (c1, median) | our OFF | our ON eager-paged | our ON GRAPHED | vLLM-ON graphed |
|---|---|---|---|---|
| Output tput (tok/s) | 10.24 | 28.65 (28.69/28.61) | **28.70 (28.70/28.70)** | 29.35 (3-rep 29.33/29.37/29.33) |
| Mean TPOT (ms) | 97.6 | 34.5 | **34.40** | 34.07 |
| ON/OFF speedup | — | 2.80x | **2.80x** | ~2.98x |
| accepted draft-tok/step | — | ~3.68 | **~3.68** | 3.44 (acc_len 4.44 - 1) |

**HONEST ATTRIBUTION (measurement supersedes the D9 hypothesis).** The D9 residual was attributed to "eager-vs-graphed launch overhead, the FULL CG closes it." REFUTED by this A/B: the CUDA graph adds only +0.3% over the eager paged path (28.70 vs 28.65, in-noise). The ACTUAL lever was Part C.1/C.2 — the paged context read ELIMINATES the D9/D12 per-layer `[context;block]` `IndexCopy` materialization of the ENTIRE growing context every layer every step (O(context) copies/layer), lifting our-ON 25.75 (D9 materialized) -> 28.65 eager-paged (+11%), which CLOSED the vLLM gap 0.917x (D9) -> **0.978x (D13)** — most of the remaining gap. **VERDICT: NEAR-PARITY, ours ~2% BELOW vLLM (0.978x).** vLLM-ON's steady-state is a tight 29.33/29.37/29.33 = 29.35 tok/s (3-rep, cold discarded); the D9 28.09 was a colder cross-session outlier, so ACROSS sessions ours (28.70) falls inside vLLM's observed 28.09-29.37 range, but on the RIGOROUS same-session band ours is ~2% below. Per the acceptance rule ("below on any axis = an open gap; near-parity is NOT met") the ≥vLLM bar is NOT met; our OFF (10.24) >= vLLM OFF; ON/OFF 2.80x (vLLM ~2.98x). **Residual attribution (data-grounded):** it is NOT acceptance (ours realized ~3.68 accepted draft-tok/step > vLLM's 3.44) and NOT launch/graph (both graphed, the CG is neutral) — it is per-step COMPUTE (the target 27B verify forward at the (1+k)=17-token batch + the 5-layer draft step + the 248320-wide lm_head), ~2% slower per target-step; the precise per-kernel breakdown needs an nsys of both draft steps (`--cuda-graph-trace=node`), the concrete next lever (no premature ceiling — same arch, closeable). The graph is LANDED + proven bit-identical (a correct capture-safe draft-step graph matching vLLM's architecture) but perf-neutral here — the roadmap's "the CG closes the gap" premise is corrected by measurement: the paged read closed most of it, the residual ~2% is per-step compute, not the graph.

**Inertness — VERIFIED on the capture binary:** 27B text SACRED `test_qwen27_paged_engine` **235/235** + 27B MTP `test_qwen27_spec_decode` **9/9** byte-identical (config-gated: the paged/graph path only runs under `use_dflash()` + P==1). CUDA `-Werror` build clean (RC 0). No new CUDA kernel (reuses the D12-sanitized `vt::DFlashPagedBlockAttention` + `IndexCopy` + `Embedding`/`MatmulBT`/`RmsNorm`/`RopeNeox`/`SiluAndMul`); compute-sanitizer covered transitively (the one from-scratch paged kernel was memcheck-0 across 795648 assertions at D12; the token-diff is the capture-safety proof). `check-device-leakage` not increased (the paged path REMOVES the materialized-buffer allocations + host uploads).
