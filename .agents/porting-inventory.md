...
15. **Extension platform: Tenstorrent Blackhole (`BACKEND-TENSTORRENT`,
    ...
    `Platform` registrar), growing toward OPT-125m: FIVE of its 9 ops now
    registered (`kMatmul`, `kMatmulBT`, `kAdd` incl. rank-1 bias broadcast,
    `kRelu`, `kEmbedding`), 7/7 test cases passing on real Blackhole
    hardware (`tests/vt/test_tenstorrent_backend.cpp`). Not yet reviewed or
    accepted by a maintainer. `ACTIVE` means a gated skeleton here, not a
    supported backend — same caveat Metal/Vulkan's own `ACTIVE` status
    carries.**
