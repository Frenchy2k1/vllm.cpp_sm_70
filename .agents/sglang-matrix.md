...
| `SGLANG-SCHED-INBATCH` | in-batch prefix-caching de-prioritization (avoid colliding on the same not-yet-cached prefix) | `schedule_policy.py:253` (`_compute_prefix_matches`), thresholds `:76`/`:83` | none; engine row `ENG-SGLANG-BEHAVIOR-FLAG` SW2 | SGLANG-DISTINCT | Follows LPM; small throughput lever on colliding prefixes. |
